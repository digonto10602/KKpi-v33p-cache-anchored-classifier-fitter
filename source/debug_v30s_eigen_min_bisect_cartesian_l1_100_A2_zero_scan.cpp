// v30s: Blind 100_A2 zero scan using closest-to-zero eigenvalue of the projected QC matrix.
...
static Eigen::MatrixXcd hermitize_v30s(const Eigen::MatrixXcd& M) {
    return 0.5*(M + M.adjoint());
}

static bool closest_zero_eigenpair_from_matrix_v30s(
    ...
    Eigen::MatrixXcd H = hermitize_v30s(M);
...
static EigMinPointV30s eigmin_point_from_eval_v30s(const EvalFull& ef) {
    EigMinPointV30s p;
    ...
    if(!closest_zero_eigenpair_from_matrix_v30s(ef.projF3inv,eig,ae,idx,nullptr,herm,emin,emax)){
...
static bool sign_flip_v30s(double a, double b) {
    return std::isfinite(a) && std::isfinite(b) && (a==0.0 || b==0.0 || a*b<0.0);
}

static bool eval_closest_eig_tracked_v30s(
        double E,
    ...
    Eigen::MatrixXcd H = hermitize_v30s(ef.projF3inv);
...
static EigZeroRecordV30s bisect_eigen_min_candidate_v30s(
        const EigMinCandidateV30s& c,
        const std::vector<int>& nnP,
        const std::string& irrep,
        const PhysicsParams& par,
        char debug,
    ...
    if(!eval_closest_eig_tracked_v30s(left,nnP,irrep,par,debug,nullptr,fl,al,&refvec,ov,herm) ||
       !eval_closest_eig_tracked_v30s(right,nnP,irrep,par,debug,(use_tracking?&refvec:nullptr),fr,ar,nullptr,ov,herm) ||
       !sign_flip_v30s(fl,fr)) {
    ...
    for(iter=0; iter<max_iter && std::abs(right-left)>Etol; ++iter) {
        ...
        if(!eval_closest_eig_tracked_v30s(mid,nnP,irrep,par,debug,(use_tracking?&refvec:nullptr),fm,am,nullptr,ovm,hm)){
    ...
    eval_closest_eig_tracked_v30s(E,nnP,irrep,par,debug,(use_tracking?&refvec:nullptr),yf,ay,nullptr,ovf,hf);
    z.E=E; z.y=yf; z.abs_y=ay; z.iter=iter; z.final_width=std::abs(right-left); z.min_overlap=std::isfinite(ovf)?std::min(minov,ovf):minov;
    z.accepted = (std::isfinite(z.abs_y) && z.final_width<=Etol*1.01) ? 1 : 0;
    z.method = use_tracking ? "tracked_eigenvector_bisection" : "instant_minabs_bisection";
    z.status = z.accepted ? "ACCEPT_EIGEN_MIN_SIGNFLIP_BISECTION" : "REJECT_BISECTION_NOT_CONVERGED";
    return z;
}

static std::vector<EigMinCandidateV30s> collect_eigen_min_signflips_v30s(
        const std::vector<EigMinPointV30s>& pts,
        const std::vector<double>& dim_jumps,
        double jump_guard) {
    std::vector<EigMinCandidateV30s> out;
    for(int i=0;i+1<(int)pts.size();++i){
        const auto& a=pts[i]; const auto& b=pts[i+1];
        if(!a.success || !b.success) continue;
        if(a.proj_dim!=b.proj_dim) continue;
        if(!sign_flip_v30s(a.eig_closest_zero,b.eig_closest_zero)) continue;
        EigMinCandidateV30s c; c.cand_index=(int)out.size(); c.i_left=i; c.i_right=i+1; c.E_left=a.E; c.E_right=b.E; c.y_left=a.eig_closest_zero; c.y_right=b.eig_closest_zero; c.proj_dim_left=a.proj_dim; c.proj_dim_right=b.proj_dim;
...
static std::vector<EigZeroRecordV30s> refine_candidates_parallel_v30s(
        const std::vector<EigMinCandidateV30s>& cands,
        const std::vector<int>& nnP,
        const std::string& irrep,
        const PhysicsParams& par,
        char debug,
        ...
        int use_tracking,
        int parallel_refine) {
    std::vector<EigZeroRecordV30s> out(cands.size());
    int done=0, nextpct=10;
    if(parallel_refine){
        #pragma omp parallel for schedule(dynamic,1)
        for(int i=0;i<(int)cands.size();++i){
            out[i]=bisect_eigen_min_candidate_v30s(cands[i],nnP,irrep,par,debug,Etol,max_iter,use_tracking);
            #pragma omp critical
            { ++done; progress_percent_log("eigmin-bisect", done, (int)cands.size(), nextpct, 10); }
        }
    } else {
        for(int i=0;i<(int)cands.size();++i){ out[i]=bisect_eigen_min_candidate_v30s(cands[i],nnP,irrep,par,debug,Etol,max_iter,use_tracking); ++done; progress_percent_log("eigmin-bisect", done, (int)cands.size(), nextpct, 10); }
    }
    int zi=0; for(auto& z: out) z.zero_index=zi++;
    return out;
}

static void write_v30s_outputs(
        const std::string& base,
        const std::vector<EigMinPointV30s>& pts,
        const std::vector<EigMinCandidateV30s>& cands,
        const std::vector<EigZeroRecordV30s>& zeros) {
    {
    ...
    }
    {
        ...
        f << "# columns: zero_index cand_index accepted Ecm eig_value abs_eig E_left E_right y_left0 y_right0 final_width iter min_overlap method status\n";
        for(const auto& z: zeros) f<<z.zero_index<<' '<<z.cand_index<<' '<<z.accepted<<' '<<z.E<<' '<<z.y<<' '<<z.abs_y<<' '<<z.E_left<<' '<<z.E_right<<' '<<z.y_left0<<' '<<z.y_right0<<' '<<z.final_width<<' '<<z.iter<<' '<<z.min_overlap<<' '<<z.method<<' '<<z.status<<"\n";
    }
    {
        std::ofstream f(base+"_eigen_min_strict_validated_levels.dat"); f<<std::setprecision(17);
        f << "# columns: level_index Ecm eig_value abs_eig final_width iter min_overlap method\n";
        int li=0;
        for(const auto& z: zeros) if(z.accepted) f<<li++<<' '<<z.E<<' '<<z.y<<' '<<z.abs_y<<' '<<z.final_width<<' '<<z.iter<<' '<<z.min_overlap<<' '<<z.method<<"\n";
    }
}

int main(int argc, char** argv) {
    if(argc!=2){ std::cerr<<"Usage:\n  "<<argv[0]<<" config/config_v30s_eigen_min_bisect_cartesian_l1_100_A2_zero_scan.in\n"; return 1; }
    const std::string input_file=argv[1];
    try{
        V30eScopedTimer total_timer("v30s total executable");
        ...
        for(const std::string& lab: opt.mom_labels){
            ...
            meta << "version = v30s_eigen_min_bisect_cartesian_l1_100_A2_zero_scan\n";
            meta << "input_file = "<<input_file<<"\nlabel = "<<lab<<"\nnnP = 0 0 1\nirrep = A2\n";
            meta << "E0 = "<<opt.E0<<"\nE1 = "<<opt.E1<<"\nN = "<<opt.N<<"\nthreads = "<<opt.threads<<"\n";
            meta << "zero_method = closest_to_zero_eigenvalue_signflip_then_bisection\n";
            ...
            meta << "note = This is a blind 100_A2 test. It does not use determinant sign flips, SVD minima, or reference energies. Coarse grid is evaluated in parallel; Vsel/projector is cached by active basis signature and reused until shell changes. Bisection uses eigenvector-overlap tracking by default, which is safer than instant min-abs branch switching.\n";

            std::cout << "[v30s] blind eigen-min bisection scan for "<<lab<<"\n";
            ...
            std::vector<EigMinPointV30s> pts(fullgrid.size());
            int nextpct=10;
            for(int i=0;i<(int)fullgrid.size();++i){ pts[i]=eigmin_point_from_eval_v30s(fullgrid[i]); progress_percent_log("eigmin-coarse", i+1, (int)fullgrid.size(), nextpct, 10); }
            ...
            auto cands=collect_eigen_min_signflips_v30s(pts,dim_jumps,opt.dimension_jump_guard);
            ...
            auto zeros=refine_candidates_parallel_v30s(cands,nnP,spec.irrep,par,opt.debug,eigen_bisect_tol,eigen_bisect_max_iter,eigen_use_tracking,opt.parallel_refine);
            ...
            write_v30s_outputs(base,pts,cands,zeros);
            stage_log("[stage 6/7] 100% outputs written");
            stage_log("[stage 7/7] done");
        }
    } catch(const std::exception& e){ std::cerr << "[v30s-error] " << e.what() << std::endl; return 2; }
    return 0;
}
