#ifndef K3DF_MINUIT_FIT_V26_REUSE_CACHE_HPP
#define K3DF_MINUIT_FIT_V26_REUSE_CACHE_HPP
...
struct K3dfFitSettings
{
    ...
    ChiSquareMode chi_square_mode = ChiSquareMode::RawResidualInverseCovariance;//NormalizedResidualInverseCorrelation;
    ...
    // v25 fitter diagnostics/cache controls.
    // This prints once per FCN evaluation. Minuit2 may call the FCN multiple
    // times per MIGRAD iteration, but this is the most robust portable hook.
    bool print_each_fcn_eval = true;
    int print_every_fcn_eval = 1;

    // Reuse projected F3^{-1} for energies already visited by coarse/refine.
    // K3 and QC are still rebuilt for every new Kdf3 parameter set.
    bool use_f3i_projected_cache = true;

    // Output files written automatically after the fit finishes.
    bool write_output_files = true;
    std::string output_dir = ".";
    std::string output_tag = "v25";
};
...
struct ModelSolveDiagnostics
{
    int n_data_levels = 0;
    int n_missing_levels = 0;
    std::map<std::string, int> data_count_by_label;
    std::map<std::string, int> qc_count_by_label;
};

inline VecD solve_model_energies_for_rows_cached(
    const K3dfFitSettings& settings,
    const K3dfParameters& params,
    const std::vector<ModelEnergyRow>& row_template,
    F3iProjectedCache* f3i_cache,
    std::map<std::string, VecD>* zeros_by_momentum_irrep_out = nullptr,
    ModelSolveDiagnostics* diagnostics_out = nullptr)
{
    VecD model(row_template.size(), 0.0);
    if (zeros_by_momentum_irrep_out) zeros_by_momentum_irrep_out->clear();

    ModelSolveDiagnostics diag;
    diag.n_data_levels = static_cast<int>(row_template.size());
    ...
    for (const MomentumIrrepSpec& spec : specs)
    {
        ...
        if (settings.use_f3i_projected_cache)
        {
            zeros = find_likely_zeros_v25_K3QC_cached(
                nnP_vec,
                spec.irrep,
                settings.coarseN,
                settings.refineN,
        ...
        {
            zeros = find_likely_zeros_v24_K3QC_silent(
                nnP_vec,
                spec.irrep,
                settings.coarseN,
                settings.refineN,
        ...
        diag.qc_count_by_label[spec.label] = static_cast<int>(zeros.size());
        ...
        for (std::size_t r = 0; r < row_template.size(); ++r)
        {
            ...
            {
                diag.data_count_by_label[spec.label] += 1;
                ...
                {
                    // User-requested convention: missing QC energies are padded by 0.0.
                    model[r] = 0.0;
                    diag.n_missing_levels += 1;
                }
                ++local_data_level;
            }
        }
    ...
    if (diagnostics_out) *diagnostics_out = diag;
    return model;
}
...
public:
    K3dfFCN(
        ...
        std::shared_ptr<F3iProjectedCache> shared_cache_)
        : settings(std::move(settings_)),
          ...
          rows(std::move(rows_)),
          shared_cache(std::move(shared_cache_))
    {}
    ...
    double operator()(const std::vector<double>& x) const override
    {
        ...
        {
            ModelSolveDiagnostics diag;
            F3iProjectedCache* cache_ptr = (settings.use_f3i_projected_cache && shared_cache)
                                             ? shared_cache.get() : nullptr;
            VecD model = solve_model_energies_for_rows_cached(settings, p, rows, cache_ptr, nullptr, &diag);
            const double chi2 = chi_square_from_model(
                data, err, model, covariance_mat, correlation_mat, settings.chi_square_mode
            );

            const double chi2_report = std::isfinite(chi2) ? chi2 : settings.failure_penalty;
            const int ndof = static_cast<int>(data.size()) - 4;
            const double red = (ndof > 0) ? chi2_report / double(ndof)
                                          : std::numeric_limits<double>::quiet_NaN();

            const std::size_t eval_id = ++fcn_eval_counter;
            if (settings.print_each_fcn_eval &&
                settings.print_every_fcn_eval > 0 &&
                (eval_id % static_cast<std::size_t>(settings.print_every_fcn_eval) == 0))
            {
                std::cout << std::setprecision(17)
                          << "[K3dfFCN-v26] eval=" << eval_id
                          << " chi2=" << chi2_report
                          << " chi2/ndof=" << red
                          << " nlevels=" << data.size()
                          << " missing=" << diag.n_missing_levels
                          << " K3iso0=" << p.K3iso0
                          << " K3iso1=" << p.K3iso1
                          << " K3B=" << p.K3B
                          << " K3E=" << p.K3E;
                if (cache_ptr)
                {
                    std::cout << " cache_size=" << cache_ptr->size()
                              << " cache_hits=" << cache_ptr->hits
                              << " cache_misses=" << cache_ptr->misses;
                }
                for (const auto& kv : diag.data_count_by_label)
                {
                    const int qc_count = diag.qc_count_by_label.count(kv.first)
                                         ? diag.qc_count_by_label.at(kv.first) : 0;
                    std::cout << " | " << kv.first << ":data=" << kv.second
                              << ",qc=" << qc_count;
                }
                std::cout << "\n";
            }
        ...
        catch (const std::exception& e)
        {
            if (settings.debug == 'y')
            {
                std::cerr << "[K3dfFCN-v26] failed at K3=("
                          << p.K3iso0 << ", " << p.K3iso1 << ", "
                          << p.K3B << ", " << p.K3E << "): "
                          << e.what() << "\n";
            }
    ...
    std::shared_ptr<F3iProjectedCache> get_shared_cache() const { return shared_cache; }
...
private:
    ...
    std::vector<ModelEnergyRow> rows;
    mutable std::shared_ptr<F3iProjectedCache> shared_cache;
    mutable std::size_t fcn_eval_counter = 0;
};
...
inline VecD finite_difference_energy_errors_and_covariance(
    ...
    F3iProjectedCache* shared_cache,
    ...
    MatrixD& model_corr_out)
{
    ...
    for (int a = 0; a < pnum; ++a)
    {
        ...
        F3iProjectedCache* cache_ptr = settings.use_f3i_projected_cache ? shared_cache : nullptr;
        VecD Ep = solve_model_energies_for_rows_cached(settings, pp, rows, cache_ptr, nullptr, nullptr);
        VecD Em = solve_model_energies_for_rows_cached(settings, pm, rows, cache_ptr, nullptr, nullptr);
...
inline std::string make_output_path(const K3dfFitSettings& settings, const std::string& stem)
{
    if (settings.output_dir.empty() || settings.output_dir == ".") return stem;
    if (settings.output_dir.back() == '/') return settings.output_dir + stem;
    return settings.output_dir + "/" + stem;
}

inline void write_matrix_dat(const std::string& path, const MatrixD& M)
{
...
inline void write_fit_outputs(const K3dfFitSettings& settings, const K3dfFitResult& r)
{
    const std::string tag = settings.output_tag;
    {
        std::ofstream out(make_output_path(settings, "fit_summary_" + tag + ".dat"));
        out << std::setprecision(17);
        out << "valid " << int(r.is_valid) << "\n";
        out << "chi2 " << r.chi2 << "\n";
        out << "ndata " << r.ndata << "\n";
        out << "npar " << r.npar << "\n";
        out << "ndof " << r.ndof << "\n";
        out << "chi2_dof " << r.chi2_dof << "\n";
    }
    {
        std::ofstream out(make_output_path(settings, "fit_levels_" + tag + ".dat"));
        out << std::setprecision(17);
        out << "# i mom_label irrep irrep_tag nPx nPy nPz state_no data_Ecm data_err model_Ecm model_err residual_sigma\n";
        for (std::size_t i = 0; i < r.rows.size(); ++i)
        {
            ...
            out << i << ' '
                ...
                << r.rows[i].irrep << ' '
                << r.rows[i].irrep_tag << ' '
                << r.rows[i].nnP[0] << ' ' << r.rows[i].nnP[1] << ' ' << r.rows[i].nnP[2] << ' '
                << r.rows[i].state_no << ' '
                << r.data_energies[i] << ' '
                << r.data_errors[i] << ' '
                << r.model_energies[i] << ' '
    ...
    {
        std::ofstream out(make_output_path(settings, "fit_parameters_" + tag + ".dat"));
        out << std::setprecision(17);
        out << "# name value error\n";
        out << "K3iso0 " << r.best.K3iso0 << ' ' << r.error.K3iso0 << "\n";
        out << "K3iso1 " << r.best.K3iso1 << ' ' << r.error.K3iso1 << "\n";
        out << "K3B "    << r.best.K3B    << ' ' << r.error.K3B    << "\n";
        out << "K3E "    << r.best.K3E    << ' ' << r.error.K3E    << "\n";
        out << "chi2 " << r.chi2 << " nan\n";
        out << "chi2_dof " << r.chi2_dof << " nan\n";
        out << "ndata " << r.ndata << " nan\n";
        out << "ndof " << r.ndof << " nan\n";
    }
    write_matrix_dat(make_output_path(settings, "fit_parameter_covariance_" + tag + ".dat"), r.parameter_covariance);
    write_matrix_dat(make_output_path(settings, "fit_parameter_correlation_" + tag + ".dat"), r.parameter_correlation);
    {
        std::ofstream out(make_output_path(settings, "fit_qc_zeros_" + tag + ".dat"));
        out << std::setprecision(17);
        out << "# mom_label zero_index Ecm\n";
        for (const auto& kv : r.zeros_by_momentum_irrep)
        {
            for (std::size_t i = 0; i < kv.second.size(); ++i)
            {
                out << kv.first << ' ' << i << ' ' << kv.second[i] << '\n';
            }
        }
    }
}

inline K3dfFitResult fit_K3df_parameters_minuit_v26(const K3dfFitSettings& settings)
{
    ...
    K3dfFCN fcn(settings, states_avg, states_err, covariance_mat, correlation_mat, rows, shared_cache);
    ...
    if (!min.IsValid())
    {
        if (settings.debug == 'y')
        {
            std::cerr << "[fit_K3df_parameters_minuit_v26] MIGRAD minimum is not valid; running HESSE anyway.\n";
        }
    }
    ...
    F3iProjectedCache* cache_ptr = settings.use_f3i_projected_cache ? shared_cache.get() : nullptr;
    result.model_energies = solve_model_energies_for_rows_cached(
        settings, result.best, rows, cache_ptr, &result.zeros_by_momentum_irrep, nullptr
    );
    ...
    {
        write_fit_outputs(settings, result);
    }
...
{
    return fit_K3df_parameters_minuit_v26(settings);
}
...
#endif // K3DF_MINUIT_FIT_V26_REUSE_CACHE_HPP
