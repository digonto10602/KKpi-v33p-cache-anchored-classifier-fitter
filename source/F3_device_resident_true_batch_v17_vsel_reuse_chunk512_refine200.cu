    ...
    char debug)
{
    ...
    std::cout << "[v17 true batch] attempting group dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " batch=" << B << " irrep=" << irrep << "\n";
    ...
    std::cout << "[v17 true batch done] dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " vdim=" << vdim << " batch=" << B
    ...
    char debug)
{
    ...
    for (int chunk_begin = 0; chunk_begin < Ecm_points; chunk_begin += energy_chunk_size)
    {
        ...
        std::cout << "[v13] chunk [" << chunk_begin << ", " << (chunk_end-1)
                  ...
                  << " v17_true_batch_attempt=enabled; Vsel_reuse_per_same_shape_group; chunk512; refine_default200; getrfBatched_plus_custom_getrs_kernel; fallback_prints_reason" << "\n";
        ...
        for (const auto& kv : groups)
        {
            ...
            if (group.size() >= 2)
            {
                std::cout << "[v17] attempting true batch for group dim1=" << key.dim1
                          << " dim2=" << key.dim2
                          << " total_dim=" << key.total_dim
                          << " count=" << group.size() << "\n";
                try
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << " # F3 true-batch grouped device-resident v17\n";
