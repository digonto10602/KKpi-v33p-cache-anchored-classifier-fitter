...
static RowOut compute_row(const fs::path& cache_path,
                          ...
                          Eigen::MatrixXcd* assembled_scaled_matrix = nullptr) {
    RowOut out;
    ...
    if(assembled_scaled_matrix) *assembled_scaled_matrix = scaled_projected_QC;
...
#ifdef V33H_BRIDGE_API
struct V33hBridgeRow {
    RowOut row;
    Eigen::MatrixXcd scaled_projected_QC;
};

static V33hBridgeRow assemble_scaled_projected_QC_for_row(const fs::path& cache_path,
                                                           const GpuRowMeta& row,
                                                           std::size_t sorted_index,
                                                           double L,
                                                           double xi,
                                                           ...
                                                           v33g_runtime_k3basis::GpuComplexReadConvention conv) {
    V33hBridgeRow out;
    out.row = compute_row(cache_path, row, sorted_index, L, xi, irrep, args, par, settings, const_norm_scale, conv, &out.scaled_projected_QC);
    return out;
}
#endif
