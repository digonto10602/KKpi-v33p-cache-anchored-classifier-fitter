    ...
    char debug)
{
    ...
    V11_CUBLAS_CHECK(cublasZgetrsBatched(handle, CUBLAS_OP_N, n, nrhs,
                                         const_cast<const cuDoubleComplex**>(dA.p), n,
                                         piv.p, dB.p, n, info.p, batch_count));

    V11_CUDA_CHECK(cudaMemcpy(hinfo.data(), info.p, sizeof(int)*hinfo.size(), cudaMemcpyDeviceToHost));
    for (int b=0; b<batch_count; ++b)
    {
        if (hinfo[static_cast<size_t>(b)] != 0)
            throw std::runtime_error("batched LU getrs failed at batch " + std::to_string(b) +
                                     " info=" + std::to_string(hinfo[static_cast<size_t>(b)]));
    }
}
    ...
    char debug)
{
    ...
    std::cout << "[v15 true batch] attempting group dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " batch=" << B << " irrep=" << irrep << "\n";
    ...
    // ---------------- Build Vsel for each energy on GPU, then pack into Vsel_batch ----------------
    std::vector<int> vdims(static_cast<std::size_t>(B), 0);
    std::vector<cuDoubleComplex*> tempV(static_cast<std::size_t>(B), nullptr);
    int common_vdim = -1;
    for (int e=0; e<B; ++e) {
        build_projection_Vsel_device(tempV[static_cast<std::size_t>(e)], vdims[static_cast<std::size_t>(e)], n, dim1,
                                     np_all[static_cast<std::size_t>(e)], nk_all[static_cast<std::size_t>(e)], irrep,
                                     nnP[0], nnP[1], nnP[2], par.parity, 0.05, 1.0e-14, 256, debug);
        if (common_vdim < 0) common_vdim = vdims[static_cast<std::size_t>(e)];
        if (vdims[static_cast<std::size_t>(e)] != common_vdim) {
            for (auto p : tempV) if (p) cudaFree(p);
            throw std::runtime_error("v15 true batch rejected: vdim is not uniform inside same-shape group");
        }
    }
    if (common_vdim <= 0) throw std::runtime_error("v15 true batch rejected: common_vdim <= 0");

    const int vdim = common_vdim;
    ...
    for (int e=0; e<B; ++e) {
        V11_CUDA_CHECK(cudaMemcpy(dVselBatch.p + static_cast<std::size_t>(e)*n*vdim,
                                  tempV[static_cast<std::size_t>(e)],
                                  sizeof(cuDoubleComplex)*static_cast<std::size_t>(n)*vdim,
                                  cudaMemcpyDeviceToDevice));
        cudaFree(tempV[static_cast<std::size_t>(e)]);
    }
    ...
    std::cout << "[v15 true batch done] dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " vdim=" << vdim << " batch=" << B
    ...
    char debug)
{
    ...
    const int energy_chunk_size = 256;
    ...
    for (int chunk_begin = 0; chunk_begin < Ecm_points; chunk_begin += energy_chunk_size)
    {
        ...
        std::cout << "[v13] chunk [" << chunk_begin << ", " << (chunk_end-1)
                  ...
                  << " v15_true_batch_attempt=enabled; batched_lu_same_n=forced_when_possible; fallback_prints_reason" << "\n";
        ...
        for (const auto& kv : groups)
        {
            ...
            if (group.size() >= 2)
            {
                std::cout << "[v15] attempting true batch for group dim1=" << key.dim1
                          << " dim2=" << key.dim2
                          << " total_dim=" << key.total_dim
                          << " count=" << group.size() << "\n";
                try
                ...
                {
                    std::cout << "[v15] true batch rejected/failed for group dim1=" << key.dim1
                              << " dim2=" << key.dim2
                              << " total_dim=" << key.total_dim
                              << " count=" << group.size()
                              << " reason: " << ex.what() << "\n";
            ...
            {
                std::cout << "[v15] true batch skipped: count < 2 for group dim1=" << key.dim1
                          << " dim2=" << key.dim2
                          << " total_dim=" << key.total_dim << "\n";
            }
            ...
            if (!used_true_batch)
            {
                std::cout << "[v15] fallback stream path active for group dim1=" << key.dim1
                          << " dim2=" << key.dim2
                          << " total_dim=" << key.total_dim
                          << " count=" << group.size() << "\n";
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << " # F3 true-batch grouped device-resident v15\n";
