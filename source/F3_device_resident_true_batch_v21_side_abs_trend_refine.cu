    ...
    char debug)
{
    ...
    std::cout << "[v21 true batch] attempting group dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " batch=" << B << " irrep=" << irrep << "\n";
    ...
    std::cout << "[v21 true batch done] dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " vdim=" << vdim << " batch=" << B
...
static int choose_projection_parity_v20(
    const std::array<int,3>& nnP,
    const std::string& irrep,
    const std::string& irrep_tag)
{
    // Match the trusted v9 CPU projection behavior for moving-frame irreps.
    // In v9, parity was set to -1 before P_irrep_projection_2plus1(...).
    // The v18 device-resident code incorrectly used +1 for labels like "A2"
    // because they do not end in "u".  That made 001/A2 appear empty.
    //
    // Rest-frame irreps with explicit g/u keep the usual parity convention.
    // Filename tags A1p/A1m are also honored when present.
    const bool rest = (nnP[0] == 0 && nnP[1] == 0 && nnP[2] == 0);

    if (!irrep.empty()) {
        const char c = irrep.back();
        if (c == 'u' || c == 'U') return -1;
        if (c == 'g' || c == 'G') return +1;
    }

    if (!irrep_tag.empty()) {
        const char c = irrep_tag.back();
        if (c == 'm' || c == 'M' || c == '-') return -1;
        if (c == 'p' || c == 'P' || c == '+') return +1;
    }

    // Moving-frame little-group labels like A1/A2/B1/B2/E carry no explicit
    // g/u suffix.  Use v9-compatible -1 by default.  For rest-frame labels
    // without g/u, also preserve the v9 default unless the tag says otherwise.
    (void)rest;
    return -1;
}
...
{
    PhysicsParams par;
    par.parity = choose_projection_parity_v20(nnP, irrep, irrep_tag);
    if (debug == 'y') {
        std::cout << "[v20 parity] nnP=[" << nnP[0] << "," << nnP[1] << "," << nnP[2]
                  << "] irrep=" << irrep << " irrep_tag=" << irrep_tag
                  << " projection_parity=" << par.parity << "\n";
    }
    ...
    for (int chunk_begin = 0; chunk_begin < Ecm_points; chunk_begin += energy_chunk_size)
    {
        ...
        std::cout << "[v13] chunk [" << chunk_begin << ", " << (chunk_end-1)
                  ...
                  << " v21_true_batch_attempt=enabled; parity_matches_v9_default_minus_for_moving_irreps; Vsel_reuse_per_same_shape_group; chunk512; refine_window_exact_signflip_pair; side_abs_trend_classifier; getrfBatched_plus_custom_getrs_kernel; fallback_prints_reason" << "\n";
        ...
        for (const auto& kv : groups)
        {
            ...
            if (group.size() >= 2)
            {
                std::cout << "[v21] attempting true batch for group dim1=" << key.dim1
                          << " dim2=" << key.dim2
                          << " total_dim=" << key.total_dim
                          << " count=" << group.size() << "\n";
                try
...
{
    // v21 side-absolute-trend classifier.
    //
    // Pick the sign flip closest to the center of the current refinement window.
    // Let the refined sign flip be between j and j+1.
    //
    // For a likely_zero:
    //   abs(det) should decrease as the left side moves inward from index 0 to j,
    //   and abs(det) should decrease as the right side moves inward from index N-1 to j+1.
    //
    // For a likely_pole:
    //   abs(det) should increase as both sides move inward toward the sign flip.
    //
    // If the two sides disagree, or either side is not decisive, classify ambiguous.
    ...
    auto abs_y = [&](int idx) -> double {
        return std::abs(rows[idx].det_norm.real());
    };

    auto classify_step = [](double outer_val, double inner_val) -> int {
        // Returns:
        //   -1 : decreasing inward, abs gets smaller toward crossing
        //    0 : flat/no useful trend
        //   +1 : increasing inward, abs gets larger toward crossing
        const double eps = 1.0e-12 * std::max({1.0, std::abs(outer_val), std::abs(inner_val)});
        if (inner_val < outer_val - eps) return -1;
        if (inner_val > outer_val + eps) return +1;
        return 0;
    };

    int left_dec = 0, left_inc = 0, left_flat = 0;
    int right_dec = 0, right_inc = 0, right_flat = 0;

    // Left side inward direction: 0 -> flip.
    // Compare abs[i] at the outer point to abs[i+1] one step closer to the sign flip.
    for (int i = 0; i < flip; ++i) {
        int t = classify_step(abs_y(i), abs_y(i+1));
        if (t < 0) left_dec++;
        else if (t > 0) left_inc++;
        else left_flat++;
    }

    // Right side inward direction: N-1 -> flip+1.
    // Compare abs[i] at the outer point to abs[i-1] one step closer to the sign flip.
    for (int i = N - 1; i > flip + 1; --i) {
        int t = classify_step(abs_y(i), abs_y(i-1));
        if (t < 0) right_dec++;
        else if (t > 0) right_inc++;
        else right_flat++;
    }

    const int left_tot = left_dec + left_inc + left_flat;
    const int right_tot = right_dec + right_inc + right_flat;
    ...
    const double left_dec_frac = double(left_dec) / double(left_tot);
    const double left_inc_frac = double(left_inc) / double(left_tot);
    const double right_dec_frac = double(right_dec) / double(right_tot);
    const double right_inc_frac = double(right_inc) / double(right_tot);

    const double avg_dec = 0.5 * (left_dec_frac + right_dec_frac);
    const double avg_inc = 0.5 * (left_inc_frac + right_inc_frac);

    out.decreasing_fraction = avg_dec;
    out.increasing_fraction = avg_inc;
    out.trend_score = avg_dec - avg_inc;

    const bool both_sides_decrease_inward =
        (left_dec_frac >= trend_required && right_dec_frac >= trend_required);

    const bool both_sides_increase_inward =
        (left_inc_frac >= trend_required && right_inc_frac >= trend_required);

    const double left_margin = std::abs(left_dec_frac - left_inc_frac);
    const double right_margin = std::abs(right_dec_frac - right_inc_frac);
    const bool decisive = (left_margin >= margin_required && right_margin >= margin_required);

    if (both_sides_decrease_inward && decisive) {
        out.classification = "likely_zero";
    } else if (both_sides_increase_inward && decisive) {
    ...
    char debug)
{
    ...
    for (int fi: flips) {
        // v20 behavior:
        // Use the exact coarse sign-flip pair as the initial refinement interval.
        // No ±window_points padding is applied.
        int il = fi;
        int ir = fi + 1;
        double EL = coarse[il].Ecm;
        double ER = coarse[ir].Ecm;
        bool done = false;
        const int Nref = refine_points;          // keep this fixed, e.g. 200
        ...
        for (int round=0; round<max_zoom_rounds && !done; ++round) {
            ...
            if (cls == "likely_zero") {
                ...
                // v20 behavior:
                // Do NOT increase N and do NOT add a window. Instead, shrink the
                // refinement interval to exactly the newest refined sign-flip pair,
                // then rerun with the same Nref.
                ...
                // v20 behavior:
                // Use exactly the latest refined sign-flip pair as the next zoom interval.
                // No ±window_points padding is applied.
                int new_il = rr.flip_left_index;
                int new_ir = rr.flip_right_index;
                ...
                std::cout << "[adaptive-device] candidate=" << rid
                          << " ambiguous -> zoom exact sign-flip pair: "
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << " # F3 true-batch grouped device-resident v21\n";
