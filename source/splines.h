/* This header file includes all the elements needed 
to construct splines for arbitrary function, we use the 
function called 'Sij_builder' that takes in the x variable 
and set of x_j points and builds the Sij matrix for each 
value of x, one needs to identify the j index of x_j for which 
x_j <= x <= x_j+1, we use this j value for Sij and sum over i, 
the function will look like F_interpolated(x) = Sum_i Sij(x) F(x_i) */
