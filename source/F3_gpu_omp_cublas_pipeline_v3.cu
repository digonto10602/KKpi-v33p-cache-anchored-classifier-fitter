...
__global__ void build_pointer_array_kernel(
    ...
    int batchCount)
{
    ...
    if (b < batchCount)
    {
        ptrs[b] = base + size_t(b) * stride;
    }
}

__global__ void build_f3_kernel(
    cuDoubleComplex* dF3,
    const cuDoubleComplex* dF2,
    const cuDoubleComplex* dF2X1,
    int elems)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < elems)
    {
        cuDoubleComplex a = dF2[idx];
        cuDoubleComplex b = dF2X1[idx];
        dF3[idx] = make_cuDoubleComplex(a.x / 3.0 - b.x, a.y / 3.0 - b.y);
    }
}

__global__ void compute_det_from_lu_kernel(
    const cuDoubleComplex* dLU,
    const int* dPivots,
    const int* dInfo,
    cuDoubleComplex* dDet,
    int n,
    int batchCount)
{
    int b = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (info != 0)
    {
        dDet[b] = make_cuDoubleComplex(NAN, NAN);
        return;
    }
    ...
    const cuDoubleComplex* LU = dLU + size_t(b) * size_t(n) * size_t(n);
    const int* piv = dPivots + size_t(b) * size_t(n);

    for (int k = 0; k < n; ++k)
    {
        cuDoubleComplex diag = LU[k + k * n];
        det = cuCmul(det, diag);

        // cuBLAS pivots are 1-based. If pivot row differs from k+1, one row swap occurred.
        if (piv[k] != k + 1)
        {
            sign = -sign;
        }
...
struct PipelineGpuCache
{
    ...
    void ensure(int n_in, int max_vdim_in, int batchCount)
    {
        ...
        CUDA_CHECK(cudaMalloc((void**)&dH, nn * sizeof(cuDoubleComplex)), "cudaMalloc dH");
        CUDA_CHECK(cudaMalloc((void**)&dF2, nn * sizeof(cuDoubleComplex)), "cudaMalloc dF2");
        CUDA_CHECK(cudaMalloc((void**)&dX1, nn * sizeof(cuDoubleComplex)), "cudaMalloc dX1");
        CUDA_CHECK(cudaMalloc((void**)&dF2X1, nn * sizeof(cuDoubleComplex)), "cudaMalloc dF2X1");
        CUDA_CHECK(cudaMalloc((void**)&dF3, nn * sizeof(cuDoubleComplex)), "cudaMalloc dF3");
        CUDA_CHECK(cudaMalloc((void**)&dF3LU, nn * sizeof(cuDoubleComplex)), "cudaMalloc dF3LU");
        CUDA_CHECK(cudaMalloc((void**)&dVsel, nv * sizeof(cuDoubleComplex)), "cudaMalloc dVsel");
        CUDA_CHECK(cudaMalloc((void**)&dX2, nv * sizeof(cuDoubleComplex)), "cudaMalloc dX2");
        CUDA_CHECK(cudaMalloc((void**)&dProj, vv * sizeof(cuDoubleComplex)), "cudaMalloc dProj");
        CUDA_CHECK(cudaMalloc((void**)&dProjLU, vv * sizeof(cuDoubleComplex)), "cudaMalloc dProjLU");
        CUDA_CHECK(cudaMalloc((void**)&dDet, size_t(cap) * sizeof(cuDoubleComplex)), "cudaMalloc dDet");

        CUDA_CHECK(cudaMalloc((void**)&dPivH, size_t(cap) * size_t(n) * sizeof(int)), "cudaMalloc dPivH");
        CUDA_CHECK(cudaMalloc((void**)&dPivF3, size_t(cap) * size_t(n) * sizeof(int)), "cudaMalloc dPivF3");
        CUDA_CHECK(cudaMalloc((void**)&dPivProj, size_t(cap) * size_t(max_vdim) * sizeof(int)), "cudaMalloc dPivProj");
        CUDA_CHECK(cudaMalloc((void**)&dInfoH, size_t(cap) * sizeof(int)), "cudaMalloc dInfoH");
        CUDA_CHECK(cudaMalloc((void**)&dInfoF3, size_t(cap) * sizeof(int)), "cudaMalloc dInfoF3");
        CUDA_CHECK(cudaMalloc((void**)&dInfoProj, size_t(cap) * sizeof(int)), "cudaMalloc dInfoProj");

        CUDA_CHECK(cudaMalloc((void**)&dH_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dH_array");
        CUDA_CHECK(cudaMalloc((void**)&dF2_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dF2_array");
        CUDA_CHECK(cudaMalloc((void**)&dX1_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dX1_array");
        CUDA_CHECK(cudaMalloc((void**)&dF2X1_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dF2X1_array");
        CUDA_CHECK(cudaMalloc((void**)&dF3_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dF3_array");
        CUDA_CHECK(cudaMalloc((void**)&dF3LU_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dF3LU_array");
        CUDA_CHECK(cudaMalloc((void**)&dVsel_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dVsel_array");
        CUDA_CHECK(cudaMalloc((void**)&dX2_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dX2_array");
        CUDA_CHECK(cudaMalloc((void**)&dProj_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dProj_array");
        CUDA_CHECK(cudaMalloc((void**)&dProjLU_array, size_t(cap) * sizeof(cuDoubleComplex*)), "cudaMalloc dProjLU_array");
    }
};
    ...
    char debug)
{
    ...
    for (int b = 0; b < batchCount; ++b)
    {
        ...
        packEigenToHostCublas(item.H, hH.data() + size_t(b) * size_t(n) * size_t(n));
        packEigenToHostCublas(item.F2, hF2.data() + size_t(b) * size_t(n) * size_t(n));
        packEigenToHostCublas(item.Vsel, hVsel.data() + size_t(b) * size_t(n) * size_t(vdim));
    }

    {
        ScopedTimer timer("GPU H2D chunk copy", debug);
        CUDA_CHECK(cudaMemcpy(cache.dH, hH.data(), nn_elems * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice), "H2D H");
        CUDA_CHECK(cudaMemcpy(cache.dF2, hF2.data(), nn_elems * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice), "H2D F2");
        CUDA_CHECK(cudaMemcpy(cache.dX1, hF2.data(), nn_elems * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice), "H2D X1 initial RHS=F2");
        CUDA_CHECK(cudaMemcpy(cache.dVsel, hVsel.data(), nv_elems * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice), "H2D Vsel");
        CUDA_CHECK(cudaMemcpy(cache.dX2, hVsel.data(), nv_elems * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice), "H2D X2 initial RHS=Vsel");
    }

    const int threads = 256;
    const int blocks_batch = (batchCount + threads - 1) / threads;

    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dH_array, cache.dH, size_t(n) * size_t(n), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dF2_array, cache.dF2, size_t(n) * size_t(n), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dX1_array, cache.dX1, size_t(n) * size_t(n), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dF2X1_array, cache.dF2X1, size_t(n) * size_t(n), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dF3_array, cache.dF3, size_t(n) * size_t(n), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dF3LU_array, cache.dF3LU, size_t(n) * size_t(n), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dVsel_array, cache.dVsel, size_t(n) * size_t(vdim), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dX2_array, cache.dX2, size_t(n) * size_t(vdim), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dProj_array, cache.dProj, size_t(vdim) * size_t(vdim), batchCount);
    build_pointer_array_kernel<<<blocks_batch, threads>>>(cache.dProjLU_array, cache.dProjLU, size_t(vdim) * size_t(vdim), batchCount);
    cudaSyncCheck("after pointer kernels");
    ...
    {
        ScopedTimer timer("GPU LU solve H X1 = F2", debug);

        CUDA_CHECK(cudaMemset(cache.dInfoH, 0, size_t(batchCount) * sizeof(int)), "memset dInfoH");
        ...
        cudaSyncCheck("after H getrfBatched");

        CUDA_CHECK(cudaMemcpy(hInfoH.data(), cache.dInfoH, size_t(batchCount) * sizeof(int), cudaMemcpyDeviceToHost), "D2H infoH");
        ...
        cudaSyncCheck("after H getrsBatched");
    }

    {
        ScopedTimer timer("GPU GEMM F2 * X1 and build F3", debug);
        ...
        cudaSyncCheck("after F2X1 gemm");

        const int elems = int(nn_elems);
        const int blocks_elems = (elems + threads - 1) / threads;
        build_f3_kernel<<<blocks_elems, threads>>>(cache.dF3, cache.dF2, cache.dF2X1, elems);
        cudaSyncCheck("after build_f3_kernel");
    }

    {
        ScopedTimer timer("GPU LU solve F3 X2 = Vsel", debug);

        CUDA_CHECK(cudaMemcpy(cache.dF3LU, cache.dF3, nn_elems * sizeof(cuDoubleComplex), cudaMemcpyDeviceToDevice), "D2D F3 to F3LU");

        CUDA_CHECK(cudaMemset(cache.dInfoF3, 0, size_t(batchCount) * sizeof(int)), "memset dInfoF3");
        ...
        cudaSyncCheck("after F3 getrfBatched");

        CUDA_CHECK(cudaMemcpy(hInfoF3.data(), cache.dInfoF3, size_t(batchCount) * sizeof(int), cudaMemcpyDeviceToHost), "D2H infoF3");
        ...
        cudaSyncCheck("after F3 getrsBatched");
    }

    {
        ScopedTimer timer("GPU GEMM Vsel^H * X2", debug);
        ...
        cudaSyncCheck("after projected gemm");
    }

    {
        ScopedTimer timer("GPU determinant LU projected matrix", debug);

        CUDA_CHECK(cudaMemcpy(cache.dProjLU, cache.dProj, vv_elems * sizeof(cuDoubleComplex), cudaMemcpyDeviceToDevice), "D2D Proj to ProjLU");

        CUDA_CHECK(cudaMemset(cache.dInfoProj, 0, size_t(batchCount) * sizeof(int)), "memset dInfoProj");
        ...
        cudaSyncCheck("after projected getrfBatched");

        CUDA_CHECK(cudaMemcpy(hInfoProj.data(), cache.dInfoProj, size_t(batchCount) * sizeof(int), cudaMemcpyDeviceToHost), "D2H infoProj");

        compute_det_from_lu_kernel<<<blocks_batch, threads>>>(
            cache.dProjLU,
            cache.dPivProj,
            cache.dInfoProj,
            cache.dDet,
        ...
        cudaSyncCheck("after compute_det_from_lu_kernel");
    }
    ...
    {
        ScopedTimer timer("GPU D2H determinant/projected", debug);
        CUDA_CHECK(cudaMemcpy(hDet.data(), cache.dDet, size_t(batchCount) * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost), "D2H det");
        CUDA_CHECK(cudaMemcpy(hProj.data(), cache.dProj, vv_elems * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost), "D2H projected matrix");
    }
    ...
    if (save_large_matrices)
    {
        ...
        ScopedTimer timer("GPU D2H X1/X2/F3", debug);
        CUDA_CHECK(cudaMemcpy(hX1.data(), cache.dX1, nn_elems * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost), "D2H X1");
        CUDA_CHECK(cudaMemcpy(hX2.data(), cache.dX2, nv_elems * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost), "D2H X2");

        CUDA_CHECK(cudaMemcpy(hF3.data(), cache.dF3, nn_elems * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost), "D2H F3");
    }
    ...
    for (int b = 0; b < batchCount; ++b)
    {
        ...
        unpackHostCublasToEigen(hProj.data() + size_t(b) * size_t(vdim) * size_t(vdim), out.F3inv_projected);
        ...
        if (save_large_matrices)
        {
            ...
            unpackHostCublasToEigen(hX1.data() + size_t(b) * size_t(n) * size_t(n), out.X1);
            unpackHostCublasToEigen(hX2.data() + size_t(b) * size_t(n) * size_t(vdim), out.X2);
            unpackHostCublasToEigen(hF3.data() + size_t(b) * size_t(n) * size_t(n), out.F3);
        }
...
{
    cublasHandle_t handle{};
    CUBLAS_CHECK(cublasCreate(&handle), "cublasCreate consumer");
