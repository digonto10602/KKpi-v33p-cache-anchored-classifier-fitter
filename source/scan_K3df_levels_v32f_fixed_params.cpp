...
static k3df_fit_v32f::FitSettings settings_from_config(const std::string& config) {
...
int main(int argc, char** argv) {
    if(argc != 2) {
        std::cerr << "Usage:\n  " << argv[0] << " config/config_v32f_fixed_K3df_spectrum.in\n";
        return 1;
    }
    ...
    try {
        ...
        std::cout << "[v32f-scan] fixed Kdf3 spectrum scan\n";
        std::cout << "[v32f-scan] K3iso0=" << std::setprecision(17) << p.K3iso0
                  << " K3iso1=" << p.K3iso1
                  << " K3B=" << p.K3B
                  << " K3E=" << p.K3E << "\n";
        std::cout << "[v32f-scan] list_of_mom:";
        for(const auto& x: s.list_of_mom) std::cout << " " << x;
        std::cout << "\n[v32f-scan] coarseN=" << s.coarseN
                  << " E=[" << s.scan_E0 << "," << s.scan_E1 << "] coarseN=" << s.coarseN << " refineN=" << s.refineN << " threads=" << s.omp_threads << "\n";

        std::cout << "[v32f-scan] [stage 1/4] building F3inv-only projected cache\n";
        auto caches = get_or_build_F3inv_cache_v32f(s);
        write_cache_grid_files(s,caches);

        std::cout << "[v32f-scan] [stage 2/4] computing closest eigenvalue grids and zero/pole levels\n";
        write_qc_eig_grid_files(s,caches,p,make_base_physics(s),"fixedK3df");

        std::cout << "[v32f-scan] [stage 3/4] writing combined spectrum file\n";
        ...
        std::cout << "[v32f-scan] wrote " << combined << "\n";

        std::cout << "[v32f-scan] [stage 4/4] 100% done\n";
    } catch(const std::exception& e) {
        std::cerr << "[v32f-scan-error] " << e.what() << "\n";
        return 2;
    }
    return 0;
}
