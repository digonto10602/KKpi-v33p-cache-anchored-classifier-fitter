#ifndef K3DF_MINUIT_FIT_V25_CACHED_HPP
#define K3DF_MINUIT_FIT_V25_CACHED_HPP
...
public:
    ...
    double operator()(const std::vector<double>& x) const override
    {
        ...
        {
            ModelSolveDiagnostics diag;
            VecD model = solve_model_energies_for_rows_cached(settings, p, rows, &f3i_cache, nullptr, &diag);
                ...
                (eval_id % static_cast<std::size_t>(settings.print_every_fcn_eval) == 0))
            {
                ...
                if (settings.use_f3i_projected_cache)
                {
                    std::cout << " cache_size=" << f3i_cache.size()
                              << " cache_hits=" << f3i_cache.hits
                              << " cache_misses=" << f3i_cache.misses;
                }
...
private:
    ...
    std::vector<ModelEnergyRow> rows;
    mutable F3iProjectedCache f3i_cache;
    ...
    MatrixD& model_corr_out)
{
    ...
    for (int a = 0; a < pnum; ++a)
    {
        ...
        F3iProjectedCache fd_cache;
        VecD Ep = solve_model_energies_for_rows_cached(settings, pp, rows, &fd_cache, nullptr, nullptr);
        VecD Em = solve_model_energies_for_rows_cached(settings, pm, rows, &fd_cache, nullptr, nullptr);
...
inline K3dfFitResult fit_K3df_parameters_minuit_v24(const K3dfFitSettings& settings)
{
    ...
    F3iProjectedCache final_cache;
    result.model_energies = solve_model_energies_for_rows_cached(
        settings, result.best, rows, &final_cache, &result.zeros_by_momentum_irrep, nullptr
    );
...
#endif // K3DF_MINUIT_FIT_V25_CACHED_HPP
