...
static SideAbsTrendResult classify_refined_file_by_side_abs_trend(
    ...
    const SideAbsRefineOptions& opt)
{
    ...
    else
    {
        ...
        else
        {
            ...
            auto classify_step = [](double outer_val, double inner_val) -> int
            {
                ...
                if (inner_val < outer_val - eps) return -1; // decreasing inward toward sign flip
                if (inner_val > outer_val + eps) return +1; // increasing inward toward sign flip
                return 0;
            };

            // Left side inward direction: index 0 -> flip_j.
            for (int i = 0; i < flip_j; ++i)
            {
            ...
            // Right side inward direction: index N-1 -> flip_j+1.
            for (int i = N - 1; i > flip_j + 1; --i)
            {
                const int t = classify_step(abs_y(i), abs_y(i - 1));
    ...
    if (!fout.is_open())
    {
        throw std::runtime_error("Could not open side-abs-trend classification output file: " + classified_output_filename);
    }

    fout << std::setprecision(17);
    fout << "# input_file = " << normalized_filename << '\n';
    fout << "# method = side_abs_inward_trend_refinement_cpu_openmp_v21\n";
    ...
    if (opt.debug == 'y')
    {
        std::cout << "[side-abs-cpu] file = " << normalized_filename
                  << ", zero_est = " << std::setprecision(17) << out.Ecm_zero_linear
                  << ", left_dec = " << out.left_decreasing_fraction
                  << ", right_dec = " << out.right_decreasing_fraction
                  << ", left_inc = " << out.left_increasing_fraction
                  << ", right_inc = " << out.right_increasing_fraction
...
void test_F3_with_pwave_all_energy_cpu_openmp_v21_side_abs_refinement(
    std::vector<int>& nnP_vec,
    std::string irrep,
    std::string irrep_tag,
    ...
    char debug = 'n')
{
    ...
    opt.duplicate_Ecm_tolerance = 1.0e-7;
    opt.debug = debug;

    const double xi = 3.444;
    const double Lbyas = 20.0;

    // No HDF5 in this CPU/OpenMP-reference v21 file.
    F3MatrixSaveOptions no_matrix_save;
    no_matrix_save.enabled = false;

    std::cout << "\n[adaptive-cpu-v21] Starting coarse "
              << coarse_Ecm_points << "-point scan.\n";
    std::cout << "[adaptive-cpu-v21] Matrix ingredients use CPU/OpenMP functions; "
              << "F3 solve uses the old v9 GPU consumer path.\n";
    std::cout << "[adaptive-cpu-v21] classifier = side_abs_inward, refine window = exact sign-flip pair\n";
    ...
    F3ScanRunFiles coarse_files = run_F3_energy_scan_gpu_omp_cublas_pipeline_v4(
        ...
        Ecm_final,
        coarse_Ecm_points,
        "_cpu_v21_coarse_N" + std::to_string(coarse_Ecm_points),
        debug,
        no_matrix_save
    );
    ...
    std::string coarse_signflip_filename =
        "classified_side_abs_cpu_v21_coarse_signflips_" +
...

    {
        ...
        fout << "# method = cpu_openmp_v21_side_abs_exact_signflip_pair\n";
    ...
    std::cout << "[adaptive-cpu-v21] Coarse sign flips found = "
              << coarse_flip_indices.size() << '\n';
    std::cout << "[adaptive-cpu-v21] Saved coarse sign-flip list to: "
    ...
    for (int coarse_flip_j : coarse_flip_indices)
    {
        ...
        std::cout << "\n[adaptive-cpu-v21] Refining candidate "
                  << refine_counter
                  << ": coarse sign flip i = ["
        ...
        for (int round = 0; round < opt.max_zoom_rounds && !done; ++round)
        {
            std::string refine_tag =
                make_scan_tag("cpu_v21_refine", refine_counter) +
                "_round" + std::to_string(round) +
                "_N" + std::to_string(Nref);

            std::cout << "[adaptive-cpu-v21] candidate=" << refine_counter
                      << " round=" << round
                      << " N=" << Nref
            ...
            SideAbsTrendResult rr = classify_refined_file_by_side_abs_trend(
            ...
            std::cout << "[adaptive-cpu-v21] candidate=" << refine_counter
                      ...
                      << " classifier=side_abs_inward_cpu_openmp"
                      << " E=" << std::setprecision(17) << ez
                      << " window=[" << refine_left << "," << refine_right << "]\n";
            ...
            else
            {
                ...
                {
                    std::cout << "[adaptive-cpu-v21] candidate=" << refine_counter
                              << " invalid sign-flip indices in zoom; stopping as ambiguous\n";
                    done = true;
                    break;
                }
                ...
                const double old_width = std::abs(refine_right - refine_left);
                ...
                refine_left = new_left;
                refine_right = new_right;
            }
        }
    ...
    std::string zeros_filename =
        ...
        std::to_string(nnP_vec[2]) +
        "_" + irrep_tag +
        "_cpu_openmp_v21_side_abs_L20.dat";
    ...
    std::cout << "\n[adaptive-cpu-v21] Finalized Ecm zeros:\n";
    ...
    fout_zero.close();
    std::cout << "[adaptive-cpu-v21] Saved finalized zero list to: "
              << zeros_filename << '\n';
}
...
int main(int argc, char** argv)
{
    try
    {
        ...
        std::cout << "# F3 CPU/OpenMP-matrix-builder v21 side-abs refinement\n";
        ...
        test_F3_with_pwave_all_energy_cpu_openmp_v21_side_abs_refinement(
            nnP_vec,
            irrep,
            irrep_tag,
            coarseN,
