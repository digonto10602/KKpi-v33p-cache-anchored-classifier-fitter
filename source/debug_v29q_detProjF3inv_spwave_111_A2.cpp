...
// v29q: input-file driven F3^{-1} zero comparison debugger.  Changes from v29o:
...
struct Options {
    ...
    std::string outdir="output_v29q";
    std::string prefix="debug_v29q_detProjF3inv_spwave_111_A2";
...
static std::vector<ZeroCandidate> collect_det_eigenbranch_candidates_from_adjacent_pairs(
    ...
    scan << "# v29q adjacent-pair eigenbranch scan summary for " << method_name(method) << "\n";
...
static void write_grid_file(const std::string& file, const std::vector<EvalData>& g, const std::string& label, const MomentumIrrepSpec& spec, const std::string& input_file) {
    std::ofstream out(file); out<<std::setprecision(17);
    out << "# v29q F3inv zero comparison grid\n# input_file "<<input_file<<"\n";
...
int main(int argc, char** argv) {
    ...
    try{
        ...
        meta << "version = v29q_detProjF3inv_spwave_111_A2\ninput_file = "<<input_file<<"\nE0 = "<<opt.E0<<"\nE1 = "<<opt.E1<<"\nN = "<<opt.N<<"\nthreads = "<<opt.threads<<"\ncomponent = real\n"
        ...
        meta << "note = v29q scans only detProjF3inv = det((V^dagger F3 V)^-1); full detF3inv zero search is disabled; F3 is positively scaled before projection to avoid p-wave overflow, preserving projected-inverse zero locations.\n";
        for(const std::string& lab: opt.mom_labels){
            ...
            std::cout<<"[v29q-detProj] scanning "<<lab<<" ...\n";
            std::cout<<"[v29q-detProj] building cached coarse projected matrices/eigensystems: N="<<opt.N<<" threads="<<opt.threads<<" coarse_parallel="<<opt.coarse_parallel<<" schedule="<<opt.omp_schedule<<" ...\n";
            ...
            const std::string base=opt.outdir+"/"+opt.prefix;
            {
                ...
                gout << "# v29q detProjF3inv sp-wave 111_A2 grid\n";
                gout << "# columns: i Ecm success total_dim_F3 proj_dim detProjF3inv_re detProjF3inv_im detProjF3inv_abs_or_logabs min_abs_eig_projF3inv min_sv_projF3 max_sv_projF3 cond_projF3 error\n";
                for(const auto& ef: fullgrid){
                    ...
                    gout << ef.d.i << ' ' << ef.d.Ecm << ' ' << (ef.d.success?1:0) << ' ' << ef.d.total_dim << ' ' << ef.d.vdim << ' '
                         ...
                         << min_abs_eig << ' ' << min_sv_projF3 << ' ' << max_sv_projF3 << ' ' << cond_projF3 << ' ' << sanitize_error(ef.d.error) << '\n';
                }
            }
            ...
            std::cout<<"[v29q-detProj] "<<lab<<" accepted detProjF3inv levels="<<levels.size()<<" expected=6\n";
            meta<<"label "<<lab<<" accepted_detProjF3inv_levels "<<levels.size()<<"\n";
        }
        std::cout<<"[v29q-detProj] wrote "<<opt.outdir<<"\n";
    } catch(const std::exception& e){ std::cerr<<"[v29q-detProj:error] "<<e.what()<<"\n"; return 2; }
    return 0;
}
