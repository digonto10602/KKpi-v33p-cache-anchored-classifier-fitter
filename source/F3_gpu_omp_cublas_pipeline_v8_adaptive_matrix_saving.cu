    ...
    std::string irrep_tag)
{
    ...
    const int classify_window = 10;        // window size
    const int refine_Ecm_points = 300;
    const double small_y_threshold = 1.0e-9;
    const double spike_ratio_threshold = 1.0e1;
    const double slope_ratio_threshold = 1.0e1;
    ...
    //
    // Default is OFF so normal adaptive scans behave like v7.
    // To save matrices, set one or both of these to true.
    //
    // Coarse saving can create a large HDF5 file because it saves many points.
    // Refinement saving is usually more useful because it saves matrices only
    // near candidate zero/pole locations.
    //==========================================================================
    ...
    std::cout << "\n[adaptive] Starting coarse" + std::to_string(coarse_Ecm_points) + "-point scan.\n";

    if (coarse_matrix_save_options.enabled)
    {
        std::cout << "[adaptive] Coarse matrix HDF5 saving enabled: "
                  << coarse_matrix_save_options.filename << '\n';
    }
    ...
    std::string coarse_classified_filename =
        "classified_zero_pole_candidates_" +
        std::to_string(nnP_vec[0]) +
        std::to_string(nnP_vec[1]) +
        std::to_string(nnP_vec[2]) +
        "_" + irrep_tag +
        "_coarse" + std::to_string(coarse_Ecm_points) + "_L20.dat";

    std::vector<SignFlipCandidate> coarse_candidates =
        classify_sign_flips_zero_vs_pole(
            coarse_files.normalized_output_filename,
            coarse_classified_filename,
            classify_window,
            small_y_threshold,
    ...
    {
        if (c.classification == "likely_pole")
        {
            if (debug == 'y')
            {
                std::cout << "[adaptive] Skipping likely pole near Ecm = "
                          << c.Ecm_zero_linear << '\n';
            }
            continue;
        }

        double refine_left = std::max(Ecm_initial, c.Ecm_window_left);
        double refine_right = std::min(Ecm_final, c.Ecm_window_right);
        ...
        std::cout << "\n[adaptive] Refining candidate " << refine_counter
                  << ": coarse_class = " << c.classification
                  << ", Ecm window = [" << refine_left << ", " << refine_right << "]"
                  << ", coarse_zero_est = " << c.Ecm_zero_linear
                  << '\n';

        std::string refine_tag = make_scan_tag("refine", refine_counter);
        ...
        {
            std::cout << "[adaptive] Refine matrix HDF5 saving enabled: "
                      << refine_matrix_save_options.filename << '\n';
        }
        ...
        std::string refine_classified_filename =
            "classified_zero_pole_candidates_" +
            std::to_string(nnP_vec[0]) +
            std::to_string(nnP_vec[1]) +
            std::to_string(nnP_vec[2]) +
        ...
        std::vector<SignFlipCandidate> refined_candidates =
            classify_sign_flips_zero_vs_pole(
                refine_files.normalized_output_filename,
                refine_classified_filename,
                classify_window,
                small_y_threshold,
                spike_ratio_threshold,
                slope_ratio_threshold,
        ...
        for (const auto& rc : refined_candidates)
        {
            if (rc.classification == "likely_zero")
            {
                add_unique_zero(finalized_Ecm_zeros, rc.Ecm_zero_linear, 1.0e-7);
                accepted_zero = true;

                std::cout << "[adaptive] Accepted refined zero: Ecm = "
                          << std::setprecision(17) << rc.Ecm_zero_linear
                          << '\n';
            }
        }

        
        if (!accepted_zero && c.classification == "likely_zero")
        {
            // Coarse looked like a zero but refinement did not give a cleaner sign flip.
            // Keep the coarse estimate but mark it in the log.
            add_unique_zero(finalized_Ecm_zeros, c.Ecm_zero_linear, 1.0e-7);
            std::cout << "[adaptive] Keeping coarse likely_zero estimate: Ecm = "
                      << std::setprecision(17) << c.Ecm_zero_linear
                      << '\n';
        }
    ...
    std::cout << "[adaptive] Saved finalized zero list to: "
              << zeros_filename << '\n';
}
