// F3_gpu_full_matrix_builder_v10.cu
//
// Purpose:
...
// GPU-safe matrix/projection builders
...
// Some of the older v7/v8/v9 files defined Vec3 and utility routines in a
// different header.  Keep local definitions here so this file is self-contained
// when included from test_F3_v10_gpu_matrix_adaptive_zeros.cu.
using Vec3 = std::array<int, 3>;

static inline comp smallest_eigenvalue(const Eigen::MatrixXcd& A)
{
    if (A.rows() != A.cols())
    {
    ...
    {
        const double abs_val = std::abs(eigvals(i));
        if (abs_val < min_abs)
        {
            min_abs = abs_val;
            min_idx = i;
...
static inline std::vector<comp> normalize_det_vector_by_max(
    ...
    char debug = 'n')
{
    if (det_proj_F3i_vec.size() != Ecm_vec.size())
    {
        throw std::runtime_error(
            "normalize_det_vector_by_max: det vector and Ecm vector must have the same size."
        );
    }

    if (det_proj_F3i_vec.empty())
    {
        throw std::runtime_error("normalize_det_vector_by_max: input determinant vector is empty.");
    }
    ...
    {
        const double abs_val = std::abs(det_proj_F3i_vec[i]);
        if (std::isfinite(abs_val) && abs_val > max_abs_det)
        {
            max_abs_det = abs_val;
            max_index = i;
        }
    ...
    {
        throw std::runtime_error(
            "normalize_det_vector_by_max: maximum determinant magnitude is zero/non-finite."
        );
    }

    const double scale = max_norm_value / max_abs_det;
    ...
    if (debug == 'y')
    {
        std::cout << std::setprecision(17)
                  << "[normalize_det_vector_by_max] max_index = " << max_index << '\n'
                  << "[normalize_det_vector_by_max] Ecm at max = " << Ecm_vec[max_index] << '\n'
                  << "[normalize_det_vector_by_max] max det value = " << det_proj_F3i_vec[max_index] << '\n'
                  << "[normalize_det_vector_by_max] max |det| = " << max_abs_det << '\n'
                  << "[normalize_det_vector_by_max] scale = " << scale << '\n';
    }
    ...
    char debug)
{
    ...
    /*
        V10 GPU-first matrix-builder path.

        The old v9 function built configs/F2/G/K2inv/projection with CPU/Eigen,
        then sent H/F2/Vsel to GPU for the F3 solve/determinant pipeline.

        This version uses the CUDA-safe companion builders:

            config_maker_4              -> f4gpu::gpu_config_maker_4_single_to_cpu_vectors
            F2_2plus1_mat              -> f2gpu::F2_2plus1_mat_gpu_safe
            G_2plus1_mat               -> ggpu::G_2plus1_mat_gpu_safe
            K2inv_EREord2_2plus1_mat   -> k2gpu::K2inv_EREord2_2plus1_mat_gpu_safe
            P_irrep_projection/Vsel    -> pgpu::P_irrep_projection_2plus1_and_Vsel_gpu_safe

        The resulting matrices are copied back to Eigen containers because the
        existing downstream v9 GPU consumer already handles chunked cuBLAS solves,
        F3 construction, projected solve, determinant, and HDF5 saving.
    */
    ...
    {
        F3ScopedTimer timer("GPU config_maker_4 both flavors", debug);
    ...
    f2gpu::F2GpuOptions f2opt;
    f2opt.threads_per_block = 256;
    f2opt.erfi_max_terms = 100;
    f2opt.erfi_tol = 1.0e-15;
    f2opt.debug = debug;
    ...
    k2gpu::Options k2opt;
    k2opt.threads_per_block = 256;
    k2opt.debug = debug;

    pgpu::Options popt;
    popt.threads_per_block = 256;
    popt.eig_tol = eig_tol;
    popt.chop_tol = 1.0e-14;
    popt.debug = debug;

    {
        F3ScopedTimer timer("GPU build F2", debug);
    ...
    {
        F3ScopedTimer timer("GPU build K2inv", debug);
    ...
    {
        F3ScopedTimer timer("GPU build G", debug);
    ...
    {
        F3ScopedTimer timer("GPU projection/Vsel", debug);

        const int nnP0 = int(std::llround(nnP_config[0].real()));
        const int nnP1 = int(std::llround(nnP_config[1].real()));
        const int nnP2 = int(std::llround(nnP_config[2].real()));
    ...
    F3MatrixSaveOptions matrix_save_options = F3MatrixSaveOptions())
{
    ...
    //=========================================================================
    // Optional HDF5 matrix dump
    //=========================================================================

    if (matrix_save_options.enabled)
    {
        F3ScopedTimer hdf5_timer("Save selected matrices to HDF5", debug);

        F3MatrixHdf5Saver saver(
            matrix_save_options.filename,
            matrix_save_options.open_mode,
            matrix_save_options.gzip_level,
            matrix_save_options.debug
        );
        ...
        for (int i = 0; i < Ecm_points; ++i)
        {
            ...
            if (!f3_should_save_matrix_point(matrix_save_options, i, r))
            {
                continue;
            }

            const bool have_F2 = (!matrix_save_options.save_F2) || (r.F2.rows() > 0 && r.F2.cols() > 0);
            const bool have_G = (!matrix_save_options.save_G) || (r.G.rows() > 0 && r.G.cols() > 0);
            const bool have_K2inv = (!matrix_save_options.save_K2inv) || (r.K2inv.rows() > 0 && r.K2inv.cols() > 0);
            const bool have_F3 = (!matrix_save_options.save_F3) || (r.F3.rows() > 0 && r.F3.cols() > 0);
            const bool have_Vsel = (!matrix_save_options.save_Vsel) || (r.Vsel.rows() > 0 && r.Vsel.cols() > 0);

            if (!(have_F2 && have_G && have_K2inv && have_F3 && have_Vsel))
            {
                if (debug == 'y')
                {
                    std::cout << "[HDF5] skipping epoint_" << i
                              << " because requested matrices are not present. "
                              << "F2=" << r.F2.rows() << "x" << r.F2.cols()
                              << ", G=" << r.G.rows() << "x" << r.G.cols()
                              << ", K2inv=" << r.K2inv.rows() << "x" << r.K2inv.cols()
                              << ", F3=" << r.F3.rows() << "x" << r.F3.cols()
                              << ", Vsel=" << r.Vsel.rows() << "x" << r.Vsel.cols()
                              << '\n';
                }
                continue;
            }
            ...
            meta.epoint_index = i;
            meta.Ecm_real = r.Ecm.real();
            meta.Ecm_imag = r.Ecm.imag();
            meta.En_real  = r.En.real();
            meta.En_imag  = r.En.imag();
            meta.L = L;
            meta.Lbyas = Lbyas;
            meta.nnP0 = nnP[0];
            meta.nnP1 = nnP[1];
            ...
            saver.save_energy_point(
                ...
                r.F3,
                r.Vsel,
                matrix_save_options.save_F2,
                matrix_save_options.save_G,
                matrix_save_options.save_K2inv,
                matrix_save_options.save_F3,
                matrix_save_options.save_Vsel,
                matrix_save_options.overwrite_existing_epoint
            );

            ++saved_count;
        }

        std::cout << "[HDF5] saved " << saved_count
                  << " energy point(s) to "
                  << matrix_save_options.filename << '\n';
    }
...
//==============================================================================
// V9 inward-shape refinement helpers
//==============================================================================

struct InwardShapeRefineOptions
{
    int coarse_window_left = 5;
    int coarse_window_right = 6;

    int initial_refine_points = 300;
    int max_refine_rounds = 4;
    int refine_growth_factor = 2;

    // These are not small-y, spike, or slope thresholds. They are only used to
    // decide whether the inward-pair sequence is dominantly increasing or
    // dominantly decreasing in the presence of numerical noise.
...
static std::vector<int> find_all_sign_flips_in_rows(
    const std::vector<DetNormRow>& rows)
{
    ...
    for (int j = 0; j + 1 < int(rows.size()); ++j)
    {
        if (f3_has_sign_flip(rows[size_t(j)].y, rows[size_t(j + 1)].y))
        {
            flips.push_back(j);
        }
    }
...
static int find_sign_flip_nearest_middle(
    const std::vector<DetNormRow>& rows)
{
...
static InwardShapeResult classify_refined_file_by_inward_shape(
    const std::string& normalized_filename,
    const std::string& classified_output_filename,
    const InwardShapeRefineOptions& opt)
{
    std::vector<DetNormRow> rows =
        read_normalized_det_file_for_sign_flips(normalized_filename);
    ...
    {
        const int flip_j = find_sign_flip_nearest_middle(rows);
        ...
        else
        {
            ...
            out.y_left = L.y;
            out.y_right = R.y;
            out.Ecm_zero_linear = linear_zero_estimate(
                L.Ecm,
                L.y,
                R.Ecm,
                R.y
            ...
            gaps.reserve(size_t(num_pairs));

            // Inward shape sequence:
            //   k=0 uses the outermost left/right points.
            //   k increases as both sides move toward the middle.
            // For a zero, this sequence should mostly decrease.
            // For a pole, this sequence should mostly increase.
            for (int k = 0; k < num_pairs; ++k)
            {
                const int il = k;
                const int ir = N - 1 - k;

                const double yl = rows[size_t(il)].y;
                const double yr = rows[size_t(ir)].y;
                ...
                gaps.push_back(std::abs(yl) + std::abs(yr));
            }

            out.num_gap_pairs = int(gaps.size());
            ...
            else
            {
                ...
                {
                    const double g0 = gaps[size_t(k)];
                    const double g1 = gaps[size_t(k + 1)];

                    const double scale = std::max({1.0, std::abs(g0), std::abs(g1)});
                    const double eps = 1.0e-12 * scale;

                    if (g1 < g0 - eps)
                    {
                        out.decreasing_steps += 1;
                    }
                    else if (g1 > g0 + eps)
                    {
                        out.increasing_steps += 1;
                    }
                    else
                    {
                        out.flat_steps += 1;
                ...
                out.num_gap_steps =
                    out.decreasing_steps + out.increasing_steps + out.flat_steps;

                if (out.num_gap_steps <= 0)
                {
                    out.classification = "ambiguous";
                }
                else
                {
                    out.decreasing_fraction =
                        double(out.decreasing_steps) / double(out.num_gap_steps);
                    out.increasing_fraction =
                        double(out.increasing_steps) / double(out.num_gap_steps);
                    out.trend_score = out.decreasing_fraction - out.increasing_fraction;

                    const double margin =
                        std::abs(out.decreasing_fraction - out.increasing_fraction);

                    if (out.decreasing_fraction >= opt.trend_fraction_required &&
                        margin >= opt.score_margin_required)
                    {
                        out.classification = "likely_zero";
                    }
                    else if (out.increasing_fraction >= opt.trend_fraction_required &&
                             margin >= opt.score_margin_required)
                    {
    ...
    {
        throw std::runtime_error(
            "Could not open inward-shape classification output file: " +
            classified_output_filename
        );
    }

    fout << std::setprecision(17);
    fout << "# input_file = " << normalized_filename << '\n';
    fout << "# method = inward_shape_gap_refinement\n";
    fout << "# trend_fraction_required = " << opt.trend_fraction_required
         << " score_margin_required = " << opt.score_margin_required << '\n';
    fout << "# signflip_local_left_index\tsignflip_local_right_index\tEcm_left\tEcm_right\t"
         << "y_left\ty_right\tEcm_zero_linear\tdecreasing_fraction\tincreasing_fraction\t"
         << "trend_score\tdecreasing_steps\tincreasing_steps\tflat_steps\tnum_gap_pairs\tclassification\n";

    fout << out.signflip_local_left_index << '\t'
         ...
         << out.increasing_fraction << '\t'
         << out.trend_score << '\t'
         << out.decreasing_steps << '\t'
         << out.increasing_steps << '\t'
         << out.flat_steps << '\t'
         << out.num_gap_pairs << '\t'
         << out.classification << '\n';
    ...
    if (opt.debug == 'y')
    {
        std::cout << "[inward-shape] file = " << normalized_filename
                  << ", zero_est = " << std::setprecision(17)
                  << out.Ecm_zero_linear
                  << ", dec_frac = " << out.decreasing_fraction
                  << ", inc_frac = " << out.increasing_fraction
                  << ", trend_score = " << out.trend_score
...
void test_F3_with_pwave_all_energy_gpu_omp_cublas_adaptive_zero_search_v1(
    std::vector<int>& nnP_vec,
    std::string irrep,
    std::string irrep_tag)
{
    ...
    const double Ecm_initial = 0.26310;
    const double Ecm_final   = 0.36;

    // Coarse scan only finds sign-flip windows. It does not decide the final zeros.
    const int coarse_Ecm_points = 2000;

    // Shape-refinement controls.
    InwardShapeRefineOptions shape_opt;
    shape_opt.coarse_window_left = 5;      // Ecm[i - 5]
    shape_opt.coarse_window_right = 6;     // Ecm[i + 6]
    shape_opt.initial_refine_points = 300; // your requested refined mesh
    shape_opt.max_refine_rounds = 4;       // 300, 600, 1200, 2400 if ambiguous
    shape_opt.refine_growth_factor = 2;
    shape_opt.trend_fraction_required = 0.65;
    shape_opt.score_margin_required = 0.20;
    shape_opt.duplicate_Ecm_tolerance = 1.0e-7;
    shape_opt.debug = debug;

    const double xi = 3.444;
    const double Lbyas = 20.0;

    //=========================================================================
    // Matrix-saving controls for this adaptive function.
    //
    // Default here follows your latest v8 file: both coarse and refinement HDF5
    // saving are enabled. Set either flag to false if you want smaller output.
    //=========================================================================

    bool save_coarse_matrices_to_hdf5 = false;
    bool save_refine_matrices_to_hdf5 = false;

    F3MatrixSaveOptions coarse_matrix_save_options;
    coarse_matrix_save_options.enabled = save_coarse_matrices_to_hdf5;
    coarse_matrix_save_options.filename =
        "F3_matrix_dump_" +
        std::to_string(nnP_vec[0]) +
        std::to_string(nnP_vec[1]) +
        std::to_string(nnP_vec[2]) +
        "_" + irrep_tag +
        "_adaptive_coarse_N" + std::to_string(coarse_Ecm_points) +
        "_L20.h5";
    coarse_matrix_save_options.open_mode = F3Hdf5OpenMode::Truncate;
    coarse_matrix_save_options.gzip_level = 4;
    coarse_matrix_save_options.debug = debug;
    coarse_matrix_save_options.save_F2 = false;
    coarse_matrix_save_options.save_G = false;
    coarse_matrix_save_options.save_K2inv = false;
    coarse_matrix_save_options.save_F3 = false;
    coarse_matrix_save_options.save_Vsel = false;
    coarse_matrix_save_options.save_every_n = 1;

    F3MatrixSaveOptions refine_matrix_save_base_options;
    refine_matrix_save_base_options.enabled = save_refine_matrices_to_hdf5;
    refine_matrix_save_base_options.open_mode = F3Hdf5OpenMode::Truncate;
    refine_matrix_save_base_options.gzip_level = 4;
    refine_matrix_save_base_options.debug = debug;
    refine_matrix_save_base_options.save_F2 = false;
    refine_matrix_save_base_options.save_G = false;
    refine_matrix_save_base_options.save_K2inv = false;
    refine_matrix_save_base_options.save_F3 = false;
    refine_matrix_save_base_options.save_Vsel = false;
    refine_matrix_save_base_options.save_every_n = 1;

    std::cout << "\n[adaptive-shape] Starting coarse "
              << coarse_Ecm_points << "-point scan.\n";

    if (coarse_matrix_save_options.enabled)
    {
        std::cout << "[adaptive-shape] Coarse matrix HDF5 saving enabled: "
                  << coarse_matrix_save_options.filename << '\n';
    }
    ...
    F3ScanRunFiles coarse_files = run_F3_energy_scan_gpu_omp_cublas_pipeline_v4(
        ...
        debug,
        coarse_matrix_save_options
    );

    // Coarse stage: only find sign-flip locations. Do not save coarse zeros.
    std::vector<DetNormRow> coarse_rows =
        read_normalized_det_file_for_sign_flips(
            coarse_files.normalized_output_filename
        );

    std::vector<int> coarse_flip_indices =
        find_all_sign_flips_in_rows(coarse_rows);

    std::string coarse_signflip_filename =
        "classified_inward_shape_coarse_signflips_" +
        std::to_string(nnP_vec[0]) +
        std::to_string(nnP_vec[1]) +
        std::to_string(nnP_vec[2]) +
        "_" + irrep_tag +
...

    {
        ...
        {
            throw std::runtime_error(
                "Could not open coarse sign-flip output file: " +
                coarse_signflip_filename
            );
        }

        fout << std::setprecision(17);
        fout << "# coarse normalized file = "
             << coarse_files.normalized_output_filename << '\n';
        fout << "# left_index\tright_index\tEcm_left\tEcm_right\tEcm_zero_linear\t"
             << "Ecm_window_left\tEcm_window_right\ty_left\ty_right\n";

        for (int j : coarse_flip_indices)
        {
            const int a = std::max(0, j - shape_opt.coarse_window_left);
            const int b = std::min(int(coarse_rows.size()) - 1,
                                   j + shape_opt.coarse_window_right);
            ...
            fout << coarse_rows[size_t(j)].i << '\t'
                 ...
                 << coarse_rows[size_t(j + 1)].Ecm << '\t'
                 << z << '\t'
                 << coarse_rows[size_t(a)].Ecm << '\t'
                 << coarse_rows[size_t(b)].Ecm << '\t'
                 << coarse_rows[size_t(j)].y << '\t'
                 << coarse_rows[size_t(j + 1)].y << '\n';
        }
    }

    std::cout << "[adaptive-shape] Coarse sign flips found = "
              << coarse_flip_indices.size() << '\n';
    std::cout << "[adaptive-shape] Saved coarse sign-flip windows to: "
              << coarse_signflip_filename << '\n';
    ...
    {
        const int a = std::max(0, coarse_flip_j - shape_opt.coarse_window_left);
        const int b = std::min(int(coarse_rows.size()) - 1,
                               coarse_flip_j + shape_opt.coarse_window_right);

        double refine_left = std::max(Ecm_initial, coarse_rows[size_t(a)].Ecm);
        double refine_right = std::min(Ecm_final, coarse_rows[size_t(b)].Ecm);

        if (refine_right < refine_left)
        {
            std::swap(refine_left, refine_right);
        }
        ...
        std::cout << "\n[adaptive-shape] Refining candidate "
                  << refine_counter
                  << ": coarse sign flip i = ["
        ...
        int refine_points = shape_opt.initial_refine_points;
        bool accepted_or_rejected = false;

        for (int round = 0; round < shape_opt.max_refine_rounds; ++round)
        {
            std::string refine_tag =
                make_scan_tag("refine", refine_counter) +
                "_round" + std::to_string(round) +
                "_N" + std::to_string(refine_points);

            F3MatrixSaveOptions refine_matrix_save_options = refine_matrix_save_base_options;
            refine_matrix_save_options.filename =
                "F3_matrix_dump_" +
                std::to_string(nnP_vec[0]) +
                std::to_string(nnP_vec[1]) +
                std::to_string(nnP_vec[2]) +
            ...
            if (refine_matrix_save_options.enabled)
            {
                std::cout << "[adaptive-shape] Refine matrix HDF5 saving enabled: "
                          << refine_matrix_save_options.filename << '\n';
            }

            std::cout << "[adaptive-shape] refine_counter = "
                      << refine_counter
                      << ", round = " << round
                      << ", N = " << refine_points
                      << ", Ecm_L = " << std::setprecision(17) << refine_left
                      << ", Ecm_R = " << refine_right
                      << '\n';
            ...
            F3ScanRunFiles refine_files = run_F3_energy_scan_gpu_omp_cublas_pipeline_v4(
                ...
                refine_tag,
                debug,
                refine_matrix_save_options
            );

            std::string refined_shape_classified_filename =
                "classified_inward_shape_" +
                std::to_string(nnP_vec[0]) +
                std::to_string(nnP_vec[1]) +
                std::to_string(nnP_vec[2]) +
                "_" + irrep_tag +
            ...
            InwardShapeResult shape_result =
                classify_refined_file_by_inward_shape(
                    refine_files.normalized_output_filename,
                    refined_shape_classified_filename,
                    shape_opt
                );

            if (shape_result.classification == "likely_zero")
            {
                add_unique_zero(
                    finalized_Ecm_zeros,
                    shape_result.Ecm_zero_linear,
                    shape_opt.duplicate_Ecm_tolerance
                );

                std::cout << "[adaptive-shape] Accepted refined likely_zero: Ecm = "
                          << std::setprecision(17)
                          << shape_result.Ecm_zero_linear
                          << " after round " << round << '\n';
            ...
            else if (shape_result.classification == "likely_pole")
            {
                std::cout << "[adaptive-shape] Rejected refined likely_pole near Ecm = "
                          << std::setprecision(17)
                          << shape_result.Ecm_zero_linear
                          << " after round " << round << '\n';
            ...
            {
                std::cout << "[adaptive-shape] Ambiguous after round "
                          << round
                          << " near Ecm = "
                          << std::setprecision(17)
                          << shape_result.Ecm_zero_linear
                          << "; refining further if allowed.\n";

                refine_points *= shape_opt.refine_growth_factor;
            }
        }

        if (!accepted_or_rejected)
        {
            std::cout << "[adaptive-shape] Candidate "
                      << refine_counter
                      << " remained ambiguous after "
                      << shape_opt.max_refine_rounds
                      << " refinement round(s). Not saving it.\n";
        }
    ...
    std::cout << "\n[adaptive-shape] Finalized Ecm zeros:\n";
    ...
    std::cout << "[adaptive-shape] Saved finalized zero list to: "
              << zeros_filename << '\n';
}
