...
// v29r: detProjF3inv sp-wave 111_A2 study with Vsel Gram-Schmidt/orthonormality diagnostics.  Changes from v29o:
...
struct Options {
    ...
    std::string prefix="debug_v30d_detProjF3inv_spwave_111_A2_whole_region";
...
static void stage_log(const std::string& msg) {
    std::cout << "[v30d-timing] t=" << std::fixed << std::setprecision(3)
              << wall_seconds_now() << " s : " << msg << std::defaultfloat << std::endl;
}

struct V30dScopedTimer {
    std::string label;
    double start;
    explicit V30dScopedTimer(std::string lab) : label(std::move(lab)), start(wall_seconds_now()) {
...
static EvalFull evaluate_full(double Ecm, const std::vector<int>& nnP_vec, const std::string& irrep, const PhysicsParams& par, char debug) {
    ...
    try {
        ...
        if(!P_I.allFinite()){ out.d.error="NONFINITE_PROJECTOR_PI"; return out; }
        out.d.P_I_herm_res = hermitian_rel_res(P_I);
        out.d.P_I_idem_res = idempotent_rel_res(P_I);
        ...
        out.d.vsel_orth_res_before = gram_orth_res(out.Vsel);
        if(g_vsel_reorthonormalize) {
            out.Vsel = modified_gram_schmidt_complex(out.Vsel, g_vsel_orth_tol);
            Pproj = out.Vsel*out.Vsel.adjoint();
        }
        out.d.vsel_orth_res_after = gram_orth_res(out.Vsel);
        out.d.Pproj_herm_res = hermitian_rel_res(Pproj);
        out.d.Pproj_idem_res = idempotent_rel_res(Pproj);
...
int main(int argc, char** argv) {
    ...
    try{
        V30dScopedTimer total_timer("total executable");
        ...
        const std::vector<double> reference_swave = {
            0.31930488895323317,
            0.33421852667982033,
            0.33760000000000000,
            0.34368263249961506,
        ...
        meta << "version = v30d_detProjF3inv_spwave_111_A2_whole_region\ninput_file = "<<input_file<<"\nE0 = "<<opt.E0<<"\nE1 = "<<opt.E1<<"\nN = "<<opt.N<<"\nthreads = "<<opt.threads<<"\ncomponent = real\n"
        ...
        meta << "\nreference_111_A2_swave_levels ="; for(double e: reference_swave) meta << ' ' << e; meta << "\n";
        meta << "note = v30d whole-energy-region N=1001 refine-grid=40 with momentum-first config_maker_4_momentum_first detProjF3inv development study with Vsel Gram-Schmidt diagnostics/reorthonormalization and residual-first acceptance = det((V^dagger F3 V)^-1); full detF3inv zero search is disabled; F3 is positively scaled before projection to avoid p-wave overflow, preserving projected-inverse zero locations.\n";
        for(const std::string& lab: opt.mom_labels){
            ...
            std::cout<<"[v30d-window] scanning "<<lab<<" ...\n";
            std::cout<<"[v30d-window] building cached coarse projected matrices/eigensystems: N="<<opt.N<<" threads="<<opt.threads<<" coarse_parallel="<<opt.coarse_parallel<<" schedule="<<opt.omp_schedule<<" ...\n";
            ...
            stage_log("writing coarse grid and Vsel diagnostics");
            {
                ...
                gout << "# v30 detProjF3inv sp-wave 111_A2 window grid\n";
                gout << "# columns: i Ecm success total_dim_F3 proj_dim detProjF3inv_re detProjF3inv_im detProjF3inv_abs_or_logabs min_abs_eig_projF3inv min_sv_projF3 max_sv_projF3 cond_projF3 vsel_orth_res_before vsel_orth_res_after P_I_herm_res P_I_idem_res Pproj_herm_res Pproj_idem_res error\n";
                for(const auto& ef: fullgrid){
                    ...
                    gout << ef.d.i << ' ' << ef.d.Ecm << ' ' << (ef.d.success?1:0) << ' ' << ef.d.total_dim << ' ' << ef.d.vdim << ' '
                         ...
                         << ef.d.Pproj_herm_res << ' ' << ef.d.Pproj_idem_res << ' ' << sanitize_error(ef.d.error) << '\n';
                }
            }
            ...
            double max_vsel_before=0.0, max_vsel_after=0.0, max_PI_herm=0.0, max_PI_idem=0.0, max_Pproj_herm=0.0, max_Pproj_idem=0.0;
            ...
            if(opt.write_vsel_diagnostics){
                ...
                vout << "# columns: i Ecm success total_dim_F3 proj_dim vsel_orth_res_before vsel_orth_res_after P_I_herm_res P_I_idem_res Pproj_herm_res Pproj_idem_res error\n";
                for(const auto& ef: fullgrid){
            ...
            }
            {
                ...
                sfile << "# columns: ref_index E_ref_swave nearest_E_spwave delta_E abs_delta_E matched match_tol\n";
                for(size_t r=0;r<reference_swave.size();++r){
                    ...
                    const int matched=(std::isfinite(delta) && delta<=opt.match_tol)?1:0;
                    sfile << r << ' ' << reference_swave[r] << ' ' << best << ' ' << (best-reference_swave[r]) << ' ' << delta << ' ' << matched << ' ' << opt.match_tol << '\n';
                }
            }
            std::cout<<"[v30d-window] "<<lab<<" accepted detProjF3inv levels="<<levels.size()<<" expected around 6 total levels from the six s-wave references\n";
            meta<<"label "<<lab<<" accepted_detProjF3inv_levels "<<levels.size()<<"\n";
        }
        std::cout<<"[v30d-window] wrote "<<opt.outdir<<"\n";
    } catch(const std::exception& e){ std::cerr<<"[v30d-window:error] "<<e.what()<<"\n"; return 2; }
    return 0;
}
