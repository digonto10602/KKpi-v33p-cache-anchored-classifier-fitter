#ifndef F3_CPU_OPENMP_V25_K3QC_CACHED_CORE_HPP
#define F3_CPU_OPENMP_V25_K3QC_CACHED_CORE_HPP
...
struct F3iProjectedCache
{
    std::map<std::string, F3iProjectedCacheEntry> table;
    std::size_t hits = 0;
    std::size_t misses = 0;
...
static std::string cache_key_v25(
    const std::vector<int>& nnP_vec,
    const std::string& irrep,
    double Ecm)
{
    std::ostringstream os;
    os << nnP_vec[0] << ',' << nnP_vec[1] << ',' << nnP_vec[2]
       << '|' << irrep
...
static F3iProjectedCacheEntry build_F3i_projected_cache_entry_v25(
    double Ecm,
    const std::vector<int>& nnP_vec,
    const std::string& irrep,
    const PhysicsParams& par)
{
    ...
    entry.dim2 = dim2;
    entry.total_dim = N;
    entry.plm_config = plm_config;
    entry.klm_config = klm_config;

    if (N <= 0)
    {
        entry.error = "empty config";
        entry.success = false;
        return entry;
    }
    ...
    entry.vdim = int(Vsel.cols());
    if (entry.vdim <= 0)
    {
        entry.error = "projection produced Vsel with zero columns";
        entry.success = false;
        return entry;
    }
    ...
    entry.Vsel = Vsel;
    entry.F3inv_projected = Vsel.adjoint() * X2;
    entry.success = true;
    return entry;
}

static EvalResult evaluate_one_energy_cpu_eigen_v25_QC_cached(
    int i,
    double Ecm,
    const std::vector<int>& nnP_vec,
    const std::string& irrep,
    ...
    char debug)
{
    ...
    try
    {
        ...
        if (cache != nullptr)
        {
            #pragma omp critical(f3i_projected_cache_access)
            {
        ...
        Eigen::MatrixXcd QC_projected = entry.F3inv_projected + K3_projected;
    ...
    catch (const std::exception& e)
    {
        ...
        if (debug == 'y')
        {
            #pragma omp critical
            {
                std::cerr << "[v25-QC-K3-cached] ERROR i=" << i
                          << " Ecm=" << std::setprecision(17) << Ecm
                          << " : " << e.what() << "\n";
            }
        }
...
static std::vector<ScanPoint> run_scan_cpu_openmp_cached(
    const std::vector<int>& nnP_vec,
    const std::string& irrep,
    double E0,
    double E1,
    ...
    char debug)
{
    ...
    for (int i = 0; i < Npts; ++i)
    {
        ...
        evals[size_t(i)] = evaluate_one_energy_cpu_eigen_v25_QC_cached(
            i, Ecm, nnP_vec, irrep, par, cache, debug
        );
    }
...
static std::vector<double> find_likely_zeros_v25_K3QC_cached(
    const std::vector<int>& nnP_vec,
    const std::string& irrep,
    int coarseN,
    int refineN,
    ...
    char debug = 'n')
{
    ...
    if (!(E1 > E0))
    {
        throw std::runtime_error("find_likely_zeros_v25_K3QC_cached requires E1 > E0");
    }

    omp_set_num_threads(par.omp_threads);
    Eigen::setNbThreads(1);

    std::vector<ScanPoint> coarse_pts = run_scan_cpu_openmp_cached(
        nnP_vec,
        irrep,
        E0,
        E1,
    ...
    while (!tasks.empty())
    {
        ...
        std::vector<ScanPoint> ref_pts = run_scan_cpu_openmp_cached(
            nnP_vec,
            irrep,
            task.E_left,
            task.E_right,
...
#endif // F3_CPU_OPENMP_V25_K3QC_CACHED_CORE_HPP
