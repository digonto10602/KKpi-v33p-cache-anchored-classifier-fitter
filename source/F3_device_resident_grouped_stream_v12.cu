    ...
    char debug)
{
    ...
    const double L = par.Lbyas / par.atmpi;
    const double pi = std::acos(-1.0);
    const double twopibyL = 2.0*pi/L;
    ...
    f4gpu::gpu_config_maker_4_single_to_cpu_vectors(
        plm_config, np_config, waves_vec_1, En_real, total_P,
        par.atmK, par.atmK, par.atmpi, L, par.epsilon_h,
        par.max_shell_num, par.tolerance, debug);

    f4gpu::gpu_config_maker_4_single_to_cpu_vectors(
        klm_config, nk_config, waves_vec_2, En_real, total_P,
        par.atmpi, par.atmK, par.atmK, L, par.epsilon_h,
        par.max_shell_num, par.tolerance, debug);
    ...
    auto worker = [&](int worker_id)
    {
        ...
        while (true)
        {
            std::size_t local = 0;
            {
                ...
                if (debug == 'y')
                {
                    std::cout << "[v12 worker " << worker_id << "] starting grouped energy "
                              << "local=" << local
                              << " i=" << group[local].i
            ...
            {
                std::lock_guard<std::mutex> lock(print_mutex);
                std::cout << "[v12 group done] i=" << group[local].i
                          << " Ecm=" << std::setprecision(17) << group[local].Ecm
                          << " det=" << out[local].det
    ...
    char debug)
{
    ...
    for (int chunk_begin = 0; chunk_begin < Ecm_points; chunk_begin += energy_chunk_size)
    {
        ...
        std::cout << "[v12] prepass chunk [" << chunk_begin << ", " << (chunk_end-1)
                  << "] tag=" << tag << "\n";
        ...
        for (int i = chunk_begin; i < chunk_end; ++i)
        {
            ...
            EnergyPrepassInfo info = get_energy_shape_prepass(
                i, Ecm, nnP, par, debug);

            if (info.total_dim <= 0)
            {
                std::cout << "[v12] skipping empty config i=" << i
                          << " Ecm=" << std::setprecision(17) << Ecm << "\n";
        ...
        std::cout << "[v12] chunk [" << chunk_begin << ", " << (chunk_end-1)
                  << "] groups=" << groups.size()
        ...
        for (const auto& kv : groups)
        {
            ...
            std::cout << "[v12] processing group dim1=" << key.dim1
                      << " dim2=" << key.dim2
                      << " total_dim=" << key.total_dim
                      << " count=" << group.size() << "\n";

            std::vector<DeviceResidentResult> gout =
                process_same_shape_group_stream_parallel(
                    group,
                    nnP,
                    irrep,
                    irrep_tag,
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << "# F3 grouped stream-parallel device-resident v12\n";
