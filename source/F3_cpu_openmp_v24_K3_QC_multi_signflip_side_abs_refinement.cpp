    ...
    char debug)
{
    ...
    const std::string raw_name =
        "det_proj_QC_K3_raw_" + tag + "_" + irrep_tag + scan_tag +
        "_cpu_only_v24_K3QC_" + ltag + ".dat";

    const std::string norm_name =
        "det_proj_QC_K3_normalized_" + tag + "_" + irrep_tag + scan_tag +
        "_cpu_only_v24_K3QC_" + ltag + ".dat";

    {
        std::ofstream f(raw_name);
        f << std::setprecision(40);
        f << "# i\tEcm\tEn\tdet_real\tdet_imag\tabs_det\ttotal_dim\tvdim\tsuccess\n";
    ...
    if (debug == 'y')
    {
        std::cout << "[v24-QC-K3] wrote " << raw_name << "\n";
        std::cout << "[v24-QC-K3] wrote " << norm_name << "\n";
    }
...
static void run_adaptive_cpu_only_v24_K3QC(
    const std::vector<int>& nnP_vec,
    const std::string& irrep,
    const std::string& irrep_tag,
    int coarseN,
    ...
    char debug)
{
    ...
    std::cout << "# QC3 K3 pure CPU/OpenMP/Eigen v24 multi-signflip side-abs refinement\n";
    std::cout << "# nnP=[" << nnP_vec[0] << "," << nnP_vec[1] << "," << nnP_vec[2] << "]"
              ...
              << " K3iso0=" << (par.K3iso.size() > 0 ? par.K3iso[0].real() : 0.0)
              << " K3iso1=" << (par.K3iso.size() > 1 ? par.K3iso[1].real() : 0.0)
              << " K3B=" << par.K3B_par.real()
              << " K3E=" << par.K3E_par.real()
              << "\n";
    std::cout << "# classifier=multi_signflip_side_abs_cpu_only_eigen_QC_equals_F3inv_plus_K3\n";

    const std::string coarse_tag =
        "_cpu_v24_K3QC_coarse_N" + std::to_string(coarseN);
    ...
    while (!tasks.empty())
    {
        ...
        stag << "_cpu_v24_K3QC_refine_" << task.candidate
             << "_branch" << task.branch
             << "_round" << task.round
             << "_N" << refineN;
    ...
    const std::string zero_file =
        ...
        "_cpu_only_v24_K3QC_multi_side_abs_" + ltag + ".dat";
...
int main(int argc, char** argv)
{
    try
    {
        ...
        if (argc > 13) K3iso0 = std::stod(argv[13]);
        if (argc > 14) K3iso1 = std::stod(argv[14]);
        if (argc > 15) K3B = std::stod(argv[15]);
        if (argc > 16) K3E = std::stod(argv[16]);

        PhysicsParams par;
        par.Lbyas = Lbyas;
        par.omp_threads = omp_threads;
        par.K3iso = {comp(K3iso0, 0.0), comp(K3iso1, 0.0)};
        par.K3B_par = comp(K3B, 0.0);
        par.K3E_par = comp(K3E, 0.0);

        // Match your trusted v9/v20 behavior for moving-frame irreps.
        par.parity = -1;

        run_adaptive_cpu_only_v24_K3QC(
            nnP_vec,
            irrep,
            irrep_tag,
            coarseN,
