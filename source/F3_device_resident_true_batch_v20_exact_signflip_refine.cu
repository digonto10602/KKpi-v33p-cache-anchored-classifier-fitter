    ...
    char debug)
{
    ...
    std::cout << "[v20 true batch] attempting group dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " batch=" << B << " irrep=" << irrep << "\n";
    ...
    std::cout << "[v20 true batch done] dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " vdim=" << vdim << " batch=" << B
    ...
    char debug)
{
    ...
    for (int chunk_begin = 0; chunk_begin < Ecm_points; chunk_begin += energy_chunk_size)
    {
        ...
        std::cout << "[v13] chunk [" << chunk_begin << ", " << (chunk_end-1)
                  ...
                  << " v20_true_batch_attempt=enabled; parity_matches_v9_default_minus_for_moving_irreps; Vsel_reuse_per_same_shape_group; chunk512; refine_window_exact_signflip_pair; getrfBatched_plus_custom_getrs_kernel; fallback_prints_reason" << "\n";
        ...
        for (const auto& kv : groups)
        {
            ...
            if (group.size() >= 2)
            {
                std::cout << "[v20] attempting true batch for group dim1=" << key.dim1
                          << " dim2=" << key.dim2
                          << " total_dim=" << key.total_dim
                          << " count=" << group.size() << "\n";
                try
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << " # F3 true-batch grouped device-resident v20\n";
