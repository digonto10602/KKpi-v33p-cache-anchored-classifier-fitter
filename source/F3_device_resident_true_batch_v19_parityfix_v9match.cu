    ...
    char debug)
{
    ...
    std::cout << "[v19 true batch] attempting group dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " batch=" << B << " irrep=" << irrep << "\n";
    ...
    std::cout << "[v19 true batch done] dim1=" << dim1
              << " dim2=" << dim2 << " n=" << n
              << " vdim=" << vdim << " batch=" << B
...
static int choose_projection_parity_v19(
    const std::array<int,3>& nnP,
    const std::string& irrep,
    const std::string& irrep_tag)
{
...
{
    PhysicsParams par;
    par.parity = choose_projection_parity_v19(nnP, irrep, irrep_tag);
    if (debug == 'y') {
        std::cout << "[v19 parity] nnP=[" << nnP[0] << "," << nnP[1] << "," << nnP[2]
    ...
    for (int chunk_begin = 0; chunk_begin < Ecm_points; chunk_begin += energy_chunk_size)
    {
        ...
        std::cout << "[v13] chunk [" << chunk_begin << ", " << (chunk_end-1)
                  ...
                  << " v19_true_batch_attempt=enabled; parity_matches_v9_default_minus_for_moving_irreps; Vsel_reuse_per_same_shape_group; chunk512; constant_refineN_zoom_window; getrfBatched_plus_custom_getrs_kernel; fallback_prints_reason" << "\n";
        ...
        for (const auto& kv : groups)
        {
            ...
            if (group.size() >= 2)
            {
                std::cout << "[v19] attempting true batch for group dim1=" << key.dim1
                          << " dim2=" << key.dim2
                          << " total_dim=" << key.total_dim
                          << " count=" << group.size() << "\n";
                try
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << " # F3 true-batch grouped device-resident v19\n";
