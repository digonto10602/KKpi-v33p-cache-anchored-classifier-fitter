...
def parse_args():
    ap=argparse.ArgumentParser(description='v32t irrep-filtered const-norm-scaled det(projF3inv) n-scale plot with nonint black and true-zero darkorange vertical lines on top')
    ap.add_argument('--output-dir', default='output_v32t_projF3inv_norm_refine_v2')
    ap.add_argument('--tag', default='debug_v32t_projF3inv_norm_refine_v2')
    ap.add_argument('--irrep', required=True, help='Full label, e.g. 000_A1m, 110_A2')
    ap.add_argument('--lattice-targets', default=None)
    ap.add_argument('--nonint-dir', default='output_nonint_v32t')
    ap.add_argument('--nonint-tag', default='debug_v32t_nonint')
...
def load_lattice(path,full_label):
    ...
    for r in read_rows(path):
        scanned+=1
        # v32t columns: row Lbyas label irrep nPx nPy nPz state E_read err_read lattice_energy_type atP shifted Ecm err
        ok=False; e=math.nan
        if len(r)>=15:
...
def main():
    ...
    for x in nonint:
        ax.axvline(x,color='black',linestyle='--',linewidth=1.1,alpha=1.0,zorder=30)
    for x in zeros:
        ax.axvline(x,color='darkorange',linestyle='--',linewidth=1.4,alpha=1.0,zorder=31)
    if lattice:
        ax.scatter(lattice,[0.0]*len(lattice),s=55,facecolors='white',edgecolors='black',linewidths=1.2,alpha=1.0,zorder=40,label='lattice Ecm')
    for n in range(nmax+1):
        yy=y*(10.0**n)
        yy[~np.isfinite(yy)]=np.nan
        ax.plot(E,yy,linewidth=1.0,alpha=1.0,zorder=5,label=(f'10^{n} f(E)' if n else 'f(E)'))
    ax.axhline(0,color='black',linewidth=0.8,alpha=1.0,zorder=25)
    ...
    ax.set_title(f'v32t true-refined digonto_classifier_v2 projected F3inv: {args.irrep}')
    ax.grid(True,alpha=1.0,linewidth=0.4,zorder=0)
    if not args.no_legend: ax.legend(fontsize=8,ncol=4)
    png=Path(args.png) if args.png else Path('output_plots')/f'v32t_{args.irrep}_projF3inv_{args.ycol}_nscale{nmax}.png'
