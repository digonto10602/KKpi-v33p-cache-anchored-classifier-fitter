#!/usr/bin/env python3
import argparse, glob, math, os, re
from pathlib import Path
import numpy as np
import matplotlib.pyplot as plt


def parse_cols(path):
    cols=None
    with open(path) as f:
        for line in f:
            if line.startswith('#') and 'columns:' in line:
                cols=line.split('columns:',1)[1].strip().split()
    if cols is None:
        raise RuntimeError(f'No # columns line found in {path}')
    return cols


def load_table(path):
    cols=parse_cols(path)
    rows=[]
    with open(path) as f:
        for line in f:
            line=line.strip()
            if not line or line.startswith('#'):
                continue
            parts=line.split()
            if len(parts) < len(cols):
                continue
            rows.append(parts[:len(cols)])
    out={c:[] for c in cols}
    for row in rows:
        for c,x in zip(cols,row):
            try:
                out[c].append(float(x))
            except ValueError:
                out[c].append(x)
    for c in cols:
        try:
            out[c]=np.asarray(out[c], dtype=float)
        except Exception:
            out[c]=np.asarray(out[c], dtype=object)
    if 'Ecm' in out and len(out['Ecm'])>1:
        E=np.asarray(out['Ecm'], dtype=float)
        finite=np.isfinite(E)
        finite_idx=np.where(finite)[0]
        nonfinite_idx=np.where(~finite)[0]
        order=np.concatenate([finite_idx[np.argsort(E[finite_idx], kind='mergesort')], nonfinite_idx])
        for c in list(out.keys()):
            if len(out[c])==len(order):
                out[c]=out[c][order]
    return cols,out


def clean_label_from_grid(path, tag):
    name=Path(path).name
    pre=f'{tag}_'
    suf='_fixedK3df_QC_closest_eig_grid.dat'
    if name.startswith(pre) and name.endswith(suf):
        return name[len(pre):-len(suf)]
    return name.replace(suf,'')


def parse_total_momentum(label):
    m=re.match(r'^(\d)(\d)(\d)_', label)
    if not m:
        return (0,0,0)
    return tuple(int(x) for x in m.groups())


def nonint_2Kpi_levels(label, mK=0.09698, mpi=0.06906, xi=3.444, Lbyas=20.0, Emin=0.261, Emax=0.36, nmax=5):
    d=parse_total_momentum(label)
    mom_unit=2.0*math.pi/(xi*Lbyas)
    d2=sum(x*x for x in d)
    out=[]
    rng=range(-nmax,nmax+1)
    vecs=[(i,j,k) for i in rng for j in rng for k in rng]
    def omega(m,n):
        n2s=sum(a*a for a in n)
        return math.sqrt(m*m + (mom_unit*mom_unit)*n2s)
    for n1 in vecs:      # K
        for n2 in vecs:  # K
            n3=(d[0]-n1[0]-n2[0], d[1]-n1[1]-n2[1], d[2]-n1[2]-n2[2]) # pi
            if any(abs(x)>nmax for x in n3):
                continue
            Elab=omega(mK,n1)+omega(mK,n2)+omega(mpi,n3)
            val=Elab*Elab - mom_unit*mom_unit*d2
            if val<=0:
                continue
            Ecm=math.sqrt(val)
            if Emin-1e-10 <= Ecm <= Emax+1e-10:
                out.append(Ecm)
    out=sorted(out)
    uniq=[]
    for x in out:
        if not uniq or abs(x-uniq[-1])>1e-8:
            uniq.append(x)
    return uniq


def load_lattice_targets(path):
    if not path or not Path(path).exists():
        return np.array([], dtype=float)
    cols,tab=load_table(path)
    key='Ecm' if 'Ecm' in tab else None
    if key is None:
        return np.array([], dtype=float)
    E=np.asarray(tab[key],dtype=float)
    return E[np.isfinite(E)]


def load_predictions(path):
    if not path or not Path(path).exists():
        return np.array([], dtype=float)
    cols,tab=load_table(path)
    if 'Ecm' not in tab:
        return np.array([], dtype=float)
    E=np.asarray(tab['Ecm'],dtype=float)
    return E[np.isfinite(E)]


def finite_xy(x,y):
    x=np.asarray(x,dtype=float); y=np.asarray(y,dtype=float)
    m=np.isfinite(x)&np.isfinite(y)
    return x[m],y[m]


def main():
    ap=argparse.ArgumentParser()
    ap.add_argument('--output-dir', default='output_qc_defaultK3df')
    ap.add_argument('--tag', default='debug_v32n_defaultK3df_QC_cache_plot')
    ap.add_argument('--lattice-targets', default='')
    ap.add_argument('--predictions', default='')
    ap.add_argument('--png', default='output_plots/v32n_defaultK3df_QC_3panel.png')
    ap.add_argument('--show', action='store_true')
    ap.add_argument('--mK', type=float, default=0.09698)
    ap.add_argument('--mpi', type=float, default=0.06906)
    ap.add_argument('--xi', type=float, default=3.444)
    ap.add_argument('--Lbyas', type=float, default=20.0)
    args=ap.parse_args()

    outdir=Path(args.output_dir)
    lattice_path=args.lattice_targets or str(outdir/f'{args.tag}_lattice_targets.dat')
    pred_path=args.predictions or str(outdir/f'{args.tag}_classifier_predictions.dat')

    pattern=str(outdir/f'{args.tag}_*_fixedK3df_QC_closest_eig_grid.dat')
    files=sorted(glob.glob(pattern))
    if not files:
        raise SystemExit(f'No grid files found with pattern: {pattern}')

    E_latt=load_lattice_targets(lattice_path)
    E_pred=load_predictions(pred_path)

    panels=[
        ('signed_slogdet_QC','signed slogdet det(projected QC)'),
        ('minAbsEig','min |eigenvalue(projected QC)|'),
        ('minSVprojQC','min SV(projected QC)'),
    ]
    fig,axes=plt.subplots(3,1,figsize=(13,10),sharex=True,constrained_layout=True)

    labels=[]
    all_E=[]
    for f in files:
        lab=clean_label_from_grid(f,args.tag)
        labels.append(lab)
        cols,tab=load_table(f)
        E=np.asarray(tab['Ecm'], dtype=float)
        if E.size:
            all_E.append(float(np.nanmin(E))); all_E.append(float(np.nanmax(E)))
        for ax,(key,ylabel) in zip(axes,panels):
            if key not in tab:
                continue
            x,y=finite_xy(E,tab[key])
            ax.plot(x,y,lw=0.85,alpha=1.0,label=lab)
            ax.set_ylabel(ylabel)
            ax.axhline(0.0,color='black',lw=0.7,linestyle='-',alpha=1.0,zorder=0)

    Emin=min(all_E) if all_E else 0.261
    Emax=max(all_E) if all_E else 0.36

    # Non-interacting 2K+pi levels: black dashed vertical lines, alpha=1.
    ni_all=[]
    for lab in labels:
        ni_all += nonint_2Kpi_levels(lab,args.mK,args.mpi,args.xi,args.Lbyas,Emin,Emax,nmax=5)
    ni_all=sorted(ni_all)
    ni_uniq=[]
    for x in ni_all:
        if not ni_uniq or abs(x-ni_uniq[-1])>2e-7:
            ni_uniq.append(x)

    for ax in axes:
        for x in ni_uniq:
            ax.axvline(x,color='black',lw=0.75,linestyle='--',alpha=1.0,zorder=1)
        for x in E_pred:
            ax.axvline(x,color='gray',lw=1.05,linestyle='--',alpha=1.0,zorder=4)
        if E_latt.size:
            ax.scatter(E_latt,np.zeros_like(E_latt),s=38,facecolors='white',edgecolors='black',marker='o',alpha=1.0,zorder=6,label='lattice Ecm')
        ax.grid(True,alpha=1.0,linestyle=':',linewidth=0.45)

    axes[-1].set_xlabel('Ecm')
    axes[0].set_title('v32n fixed/default K3df projected QC diagnostics from cached F3inv')
    handles,labtxt=axes[0].get_legend_handles_labels()
    if handles:
        axes[0].legend(handles[:10],labtxt[:10],loc='best',fontsize=8,ncol=2)

    Path(args.png).parent.mkdir(parents=True,exist_ok=True)
    fig.savefig(args.png,dpi=240)
    print(f'[plot-v32n] wrote {args.png}')
    print(f'[plot-v32n] lattice targets: {len(E_latt)} from {lattice_path}')
    print(f'[plot-v32n] classifier predicted lines: {len(E_pred)} from {pred_path}')
    if args.show:
        plt.show()

if __name__=='__main__':
    main()
